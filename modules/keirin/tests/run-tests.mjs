...
assert.ok(flat.purchasePlan.length>=1);
assert.equal(flat.noBet,true);
assert.equal(flat.noBetReason,"LINE_FALLBACK_INSUFFICIENT_DISCRIMINATION");
assert.equal(flat.audit.referencePlan,true);
assert.equal(flat.purchasePlan.every(item=>item.referenceOnly===true),true);
