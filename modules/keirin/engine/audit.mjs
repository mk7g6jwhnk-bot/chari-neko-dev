...
export function audit({race,branches,terminals}){
  ...
  return{passed:errors.length===0,errors,warnings:["終端削除なし",`全終端${terminals.length}件保持`,race.lineConfidence!=="高"?"公式ライン未取得: ライン非依存の先行/まくり枝で継続し、番手差し等のライン依存枝のみ停止":null].filter(Boolean),probabilitySum:sum,unterminatedBranches:errors.filter(x=>x.includes("終端なし")).length};
}
