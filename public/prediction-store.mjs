...
export function evaluateResult(snapshot,official,now=new Date()){const status=normalizeStatus(official.status);if(status==="not_finished")throw new Error("公式結果はまだ確定していません");const order=(official.finishOrder||official.order||[]).slice(0,3).map(Number);const exceptional=detectExceptionalRace(official);const learningDisposition=exceptional?{mode:"EXCEPTIONAL_SEPARATE",includeInNormalLearning:false,reason:exceptional.reason,details:exceptional.details}:{mode:"NORMAL",includeInNormalLearning:true,reason:null,details:[]};if(status==="cancelled"||status==="refund")return {resultStatus:status,officialFinishOrder:order,officialPayout:official.payout??null,matchedSelection:null,betCategory:null,predictionSnapshotId:snapshot.predictionSnapshotId,checkedAt:now.toISOString(),learningDisposition};if(order.length<3)throw new Error("公式の確定着順を取得できません");const key=order.join("-");const matched=(snapshot.betSelections||[]).find(b=>b.order.join("-")===key)||null;const terminal=(snapshot.terminalLedger||[]).find(t=>(t.order||[]).join("-")===key)||null;return {resultStatus:matched?"hit":"miss",officialFinishOrder:order,officialPayout:official.payout??null,matchedSelection:matched?.order||null,betCategory:matched?.category||null,terminalWasGenerated:Boolean(terminal),terminalProbability:terminal?.probability??null,terminalPurchaseStatus:terminal?.purchaseStatus||null,terminalRejectCode:terminal?.purchaseRejectCode||null,terminalRejectReason:terminal?.purchaseReason||null,predictionSnapshotId:snapshot.predictionSnapshotId,checkedAt:now.toISOString(),learningDisposition}}
...
function compactSnapshot(s,aggressive){
  ...
  return {...s,
    ...
    abilitiesUsed:(s.abilitiesUsed||[]).map(a=>({number:a.number,recentForm:a.recentForm??null,startPower:a.startPower??null,sprintPower:a.sprintPower??null,finishPower:a.finishPower??null,trackingSkill:a.trackingSkill??null,roleScores:a.roleScores||null,abilityMissingAudit:a.abilityMissingAudit||null})),
