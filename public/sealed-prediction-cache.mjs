const DB_NAME="chari-neko-sealed-predictions",STORE="payloads",RESULT_STORE="results",VERSION=2,METRICS_KEY="chari-neko:read-metrics:v1";
...
export async function getSealedResultCache(raceKey,predictionHash,lifecycleVersion="1"){
  if(!predictionHash)return null;const db=await openDb();return request(db.transaction(RESULT_STORE,"readonly").objectStore(RESULT_STORE).get(`${raceKey}|${predictionHash}|${lifecycleVersion}`));
}
export async function putSealedResultCache(raceKey,predictionHash,lifecycleVersion,payload){
  if(!predictionHash)return;const db=await openDb(),tx=db.transaction(RESULT_STORE,"readwrite"),key=`${raceKey}|${predictionHash}|${lifecycleVersion}`;tx.objectStore(RESULT_STORE).put({key,raceKey,predictionHash,lifecycleVersion,payload,cachedAt:new Date().toISOString()});await transaction(tx);
}
...
function openDb(){return new Promise((resolve,reject)=>{const r=indexedDB.open(DB_NAME,VERSION);r.onupgradeneeded=()=>{if(!r.result.objectStoreNames.contains(STORE))r.result.createObjectStore(STORE,{keyPath:"key"});if(!r.result.objectStoreNames.contains(RESULT_STORE))r.result.createObjectStore(RESULT_STORE,{keyPath:"key"})};r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)})}
