...
function renderBetGroups(snapshot){
  ...
  const groupHtml=groups.map(([label,key])=>{
    ...
    if(rows.length){const families=new Map();for(const row of rows){const id=row.b.originatingScenarioFamily||row.b.scenarioFamilyLabel||row.b.branchLabel||"UNRESOLVED";if(!families.has(id))families.set(id,[]);families.get(id).push(row)}const familyHtml=[...families.entries()].map(([,familyRows])=>{const first=familyRows[0].b,familyLabel=first.scenarioFamilyLabel||first.branchLabel||"展開未確認",meta=Number.isFinite(Number(first.scenarioFamilyProbability))?`family確率 ${(Number(first.scenarioFamilyProbability)*100).toFixed(1)}%`:"family確率 未確認";return `<div class="scenarioFamilyGroup"><h4>${esc(familyLabel)}</h4><p class="muted">${meta}${first.mainDifferenceReason?` / ${esc(first.mainDifferenceReason)}`:""}</p><div class="betRows">${familyRows.map(({b,i})=>`<div class="betRow betRowSimple"><strong>${b.order.join("-")}${thickKeys.has(b.order.join("-"))?"　🔥厚め":""}</strong><span>${b.odds?`${Number(b.odds)}倍`:"オッズ確認待ち"}${stakes?` / ${Number(stakes[i]).toLocaleString()}円`:" / 配分不可"}</span></div>`).join("")}</div></div>`}).join("");return `<section class="betCard"><h3>${label}</h3>${familyHtml}</section>`}
