...
async function loadCollectorStatus(){if(state.collectorStatusBusy)return;state.collectorStatusBusy=true;let cached=loadCachedCollectorStatus(localStorage);if(cached&&!state.collectorStatus){state.collectorStatus=cached;renderCollectorStatus();renderPredictionPerformance()}try{const p=await jsonFetch("/.netlify/functions/keirin-collector-status");saveCachedCollectorStatus(localStorage,p);cached=loadCachedCollectorStatus(localStorage);state.collectorStatus=collectorStatusViewModel(p,cached).status||p;renderCollectorStatus();renderPredictionPerformance()}catch(error){state.collectorStatus=cached?{...cached,statusReadFailed:true,statusReadError:error?.message||String(error)}:{ok:false,statusReadFailed:true,error:error?.message||String(error)};renderCollectorStatus();renderPredictionPerformance()}finally{state.collectorStatusBusy=false}}
function renderPredictionPerformance(){const root=$("predictionPerformance"),report=state.collectorStatus?.purchasePerformance;if(!root)return;if(!report?.periods){root.innerHTML='<div class="sectionHead"><h2>予想成績</h2><span class="pill">集計中</span></div><p class="muted">正式sealと確定結果が揃うまで「0」ではなく集計中として表示します。</p>';return}const renderPeriod=(label,p)=>{const pct=v=>Number.isFinite(Number(v))?`${(Number(v)*100).toFixed(1)}%`:"—",yen=v=>Number.isFinite(Number(v))?`${Number(v).toLocaleString("ja-JP")}円`:"—",category=(name,key)=>{const c=p?.categories?.[key];return[`${name}的中率`,pct(c?.hitRate),`${name}回収率`,pct(c?.roi)]};return`<h3>${label}</h3>${metas([["対象",p?.evaluatedRaces??"—"],["購入対象",p?.betRaces??"—"],["的中",p?.hitRaces??"—"],["的中率",pct(p?.hitRate)],["100円平買い投資",yen(p?.investment)],["払戻",yen(p?.return)],["回収率",pct(p?.roi)],category("本線","main").slice(0,2),category("本線","main").slice(2),category("押さえ","cover").slice(0,2),category("押さえ","cover").slice(2),category("厚め","thick").slice(0,2),category("厚め","thick").slice(2)])}`};root.innerHTML=`<div class="sectionHead"><h2>予想成績</h2><span class="pill success">正式sealのみ</span></div>${renderPeriod("今日",report.periods.today)}${renderPeriod("累積",report.periods.cumulative)}`}
