...
const APP_RELEASE="KEIRIN-0.5.22-meeting-last-known-fallback";
...
function renderStatusSurfaces(){renderCollectorStatus();renderAutoCollectionSummary();renderPredictionPerformance();renderHomePredictionStatus();renderHomeSystemStatus()}
...
function raceStatus(r){if(r?.resultConfirmed||r?.resultStatus==="RESULT_CONFIRMED")return{label:"終了",className:"danger"};const t=parseTime(r.date,deadlineOf(r));if(!t)return{label:r?.officialStatus==="OFFICIAL_STATUS_CONFIRMED"?"未発走":"状態未確認",className:""};const diff=t-Date.now();if(diff<=0)return{label:"結果確認中",className:"warning"};if(diff<=15*60000)return{label:"締切間近",className:"warning"};return{label:"未発走",className:""}}
