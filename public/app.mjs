...
async function loadCollectorStatus(){if(state.collectorStatusBusy)return;state.collectorStatusBusy=true;let cached=loadCachedCollectorStatus(localStorage);if(cached&&!state.collectorStatus){state.collectorStatus=cached;renderCollectorStatus()}try{const p=await jsonFetch("/.netlify/functions/keirin-collector-status");saveCachedCollectorStatus(localStorage,p);cached=loadCachedCollectorStatus(localStorage);state.collectorStatus=collectorStatusViewModel(p,cached).status||p;renderCollectorStatus()}catch(error){state.collectorStatus=cached?{...cached,statusReadFailed:true,statusReadError:error?.message||String(error)}:{ok:false,statusReadFailed:true,error:error?.message||String(error)};renderCollectorStatus()}finally{state.collectorStatusBusy=false}}
