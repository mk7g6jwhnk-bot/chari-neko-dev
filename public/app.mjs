...
void loadCollectorStatus();void loadPredictionHistory({homeOnly:true});
setupAutoUpdate();
...
async function loadCollectorStatus(){if(state.collectorStatusBusy)return;state.collectorStatusBusy=true;let cached=loadCachedCollectorStatus(localStorage);if(cached&&!state.collectorStatus){state.collectorStatus=cached;renderStatusSurfaces()}try{const p=await jsonFetch("/.netlify/functions/keirin-collector-status");state.collectorStatus=p;renderStatusSurfaces();saveCachedCollectorStatus(localStorage,p)}catch(error){if(!state.collectorStatus||state.collectorStatus===cached){state.collectorStatus=cached?{...cached,statusReadFailed:true,statusReadError:error?.message||String(error)}:{ok:false,statusReadFailed:true,error:error?.message||String(error)};renderStatusSurfaces()}}finally{state.collectorStatusBusy=false}}
function renderStatusSurfaces(){renderCollectorStatus();renderAutoCollectionSummary();renderPredictionPerformance();renderHomePredictionStatus();renderHomeSystemStatus();if(state.meetings.length)renderMeetingTabs()}
function renderAutoCollectionSummary(){const root=$("collectorProgress"),s=state.collectorStatus;if(!root||!s)return;const value=(number,suffix="")=>number===null||number===undefined||!Number.isFinite(Number(number))?"—":`${Number(number)}${suffix}`,block=document.createElement("div");block.className="autoCollectionSummary";block.innerHTML=metas([["開催",value(s.meetingCount)],["対象",value(s.todayRaceCount,"R")],["prefetch",value(s.prefetchedRaceCount,"R")],["発走前seal",value(s.preRaceSealSucceeded??s.sealedPredictionCount,"R")],["seal失敗",value(s.preRaceSealFailed,"R")],["発走前未達",value(s.missedBeforeStart,"R")]]);root.querySelector(".autoCollectionSummary")?.remove();root.querySelector(".sectionHead")?.insertAdjacentElement("afterend",block)}
function renderPredictionPerformance(){const root=$("predictionPerformance"),report=state.collectorStatus?.purchasePerformance;if(!root)return;if(!report?.periods){root.innerHTML='<div class="sectionHead"><h2>予想成績</h2><span class="pill">集計中</span></div><p class="muted">正式sealと確定結果が揃うまで「0」ではなく集計中として表示します。</p>';return}const pct=v=>v===null||v===undefined||!Number.isFinite(Number(v))?"—":`${(Number(v)*100).toFixed(1)}%`,yen=v=>v===null||v===undefined||!Number.isFinite(Number(v))?"—":`${Number(v).toLocaleString("ja-JP")}円`,renderPeriod=(label,p)=>{if(!p||p.evaluatedRaces===0)return`<h3>${label}</h3><p class="muted">まだ集計対象レースがありません</p>`;const rows=[["対象",`${p.evaluatedRaces}R`],["購入対象",`${p.betRaces}R`],["的中",`${p.hitRaces}R`],["的中率",pct(p.hitRate)],["100円平買い投資",yen(p.investment)],["100円平買い払戻",yen(p.return)],["100円平買い回収率",pct(p.roi)]];for(const [name,key] of [["本線","main"],["押さえ","cover"],["厚め","thick"]]){const c=p.categories?.[key];rows.push([`${name} 点数`,c?`${c.ticketCount}点`:"—"],[`${name} 的中率`,pct(c?.hitRate)],[`${name} 回収率`,pct(c?.roi)])}rows.push(["実stake",p.actualStake?.available?`${yen(p.actualStake.investment)} → ${yen(p.actualStake.return)} / ${pct(p.actualStake.roi)}`:"保存stakeなし"]);return`<h3>${label}</h3>${metas(rows)}`},start=report.statsEligibleFrom?`集計開始: ${formatTime(report.statsEligibleFrom)} / ${esc(report.firstEligibleRaceKey||"")}`:"購入判定を保存できたレースから集計";root.innerHTML=`<div class="sectionHead"><h2>予想成績</h2><span class="pill success">PURCHASE_PERFORMANCE_V2</span></div><p class="muted">購入判定を保存できたレースから集計します。${start}</p>${renderPeriod("今日",report.periods.today)}${renderPeriod("直近7日",report.periods.recent7)}${renderPeriod("直近30日",report.periods.recent30)}${renderPeriod("累積",report.periods.cumulative)}`}
function renderCollectorStatus(){const root=$("collectorProgress"),p=state.collectorStatus;if(!root)return;if(!p){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill">確認中</span></div>';return}const cached=loadCachedCollectorStatus(localStorage),view=collectorStatusViewModel(p,cached);if(view.mode==="STATUS_UNAVAILABLE"){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill warning">状態を確認できません</span></div><p class="muted">現在値も保存済み情報もありません。件数は確認できません。</p>';return}const s=view.status,autoStale=view.autoMode.startsWith("STALE"),researchStale=view.researchMode==="STALE",storageBlocked=view.autoMode.includes("STORAGE_BLOCKED"),actualError=storageBlocked||view.autoMode==="COLLECTOR_ERROR"||view.autoMode==="STALE_COLLECTOR_ERROR",confirmed=s.autoObservedAt||s.lastResultRunAt||s.lastPredictionRunAt,healthy=view.autoMode==="UNAVAILABLE"?"自動収集：確認できません":storageBlocked?"自動収集：保存停止中":actualError?(autoStale?"前回確認時エラー":"自動収集エラー"):(autoStale?`正常稼働中（前回確認 ${confirmed?formatTime(confirmed):"時刻不明"}）`:"正常稼働中"),updated=s.lastSuccessfulCollectorUpdate||s.lastResultRunAt||s.lastPredictionRunAt,r=s.researchProgress,metric=(value,suffix="R")=>value===null||value===undefined?"確認できません":`${value}${suffix}`,progress=(value,target)=>value===null||value===undefined?"確認できません":`${value} / ${target}`,storageNotice=storageBlocked?'<p class="muted">ストレージ容量不足のため、新しい予想・結果・研究観測を保存できません。</p>':"",notice=(autoStale||researchStale||view.autoMode==="UNAVAILABLE"||view.researchMode==="UNAVAILABLE")?'<p class="muted">取得できなかった項目は、同じsourceの保存済みsnapshotを表示しています。保存値もない項目は「確認できません」と表示します。</p>':"";root.innerHTML=`<div class="sectionHead"><h2>今日の自動収集</h2><span class="pill ${actualError||view.autoMode==="UNAVAILABLE"?"warning":"success"}">${healthy}</span></div>${storageNotice}${notice}${metas([["予想取得済み",metric(s.sealedPredictionCount)],["結果取得済み",metric(s.resultLoadedCount)],["比較済み",metric(s.verifiedCount)],["結果待ち",metric(s.pendingResultCount)],["再試行中",metric(s.retryingCount)],["最終更新",updated?formatTime(updated):"確認できません"]])}<h3>累積研究評価${researchStale&&s.researchObservedAt?`（前回確認 ${formatTime(s.researchObservedAt)}）`:""}</h3>${metas([["比較可能",metric(r?.comparableTotal)],["50R監査",progress(r?.progress50,50)],["100R監査",progress(r?.progress100,100)],["Cohort",r?.cohort||"確認できません"],["Report",r?.reportStatus||"確認できません"]])}${s.lastError?`<details><summary>詳細</summary><p class="muted">${esc(typeof s.lastError==="string"?s.lastError:`${s.lastError.code||"ERROR"} / ${s.lastError.raceKey||"race不明"} / ${s.lastError.stage||"stage不明"}`)}</p></details>`:""}`}
...
function renderHomeSystemStatus(){const root=$("homeSystemStatus"),raw=state.collectorStatus;if(!root)return;const view=collectorStatusViewModel(raw,loadCachedCollectorStatus(localStorage));if(!view.status){root.innerHTML='<div class="sectionHead"><h2>研究・システム状態</h2><span class="pill warning">状態を確認できません</span></div><p class="muted">現在値も保存済み値もありません。</p>';return}const s=view.status,r=s.researchProgress||{},stale=view.autoSourceLevel!=="current"||view.researchSourceLevel!=="current",source=view.autoSourceLevel==="server-last-known"?"最新取得失敗・保存済み値":view.autoSourceLevel==="browser-last-known"?"端末保存値":"現在値",updated=s.lastSuccessfulCollectorUpdate||s.autoObservedAt||s.researchObservedAt,age=updated?relativeAge(updated):"不明";root.innerHTML=`<div class="sectionHead"><h2>研究・システム状態</h2><span class="pill ${stale?"warning":"success"}">${stale?"stale":"current"}</span></div>${metas([["Research compared",r.comparableTotal??"—"],["collector",s.collectorOperational===true?"稼働":s.collectorOperational===false?"停止":"不明"],["last update",updated?`${age}前`:"—"],["source",source],["errors",s.failureCount??"—"]])}<details><summary>運用詳細</summary>${metas([["開催",s.meetingCount??"—"],["対象",s.todayRaceCount??"—"],["prefetch",s.prefetchedRaceCount??"—"],["seal",s.preRaceSealSucceeded??s.sealedPredictionCount??"—"],["queue",s.retryingCount??"—"],["storage",s.storageWritable===true?"writable":s.storageWritable===false?"blocked":"unknown"],["browser",s.browserConnected===true?"connected":s.browserConnected===false?"disconnected":"unknown"],["raw status",view.autoSourceLevel]])}</details>`}
...
function historyOutcome(row){if(!row.resultAttached)return"waiting";if(row.purchaseEvaluation?.purchaseEligibility!=="PURCHASE_ALLOWED")return"blocked";return row.purchaseEvaluation?.standardHit?"hit":"miss"}
...
function historyCard(row,index){const stateLabel=historyState(row),evaluation=row.purchaseEvaluation||{},result=(row.finishOrder||[]).join("-")||"結果待ち",outcome=historyOutcome(row),outcomeLabel={hit:"的中",miss:"不的中",waiting:"結果待ち",blocked:"評価対象外"}[outcome],money=value=>value==null?"—":`${Number(value).toLocaleString()}円`,roi=value=>Number.isFinite(Number(value))?`${(Number(value)*100).toFixed(1)}%`:"—";return `<article class="card compact historyCard"><button class="historyOpen" data-history="${index}"><div class="sectionHead"><strong>${esc(formatDate(String(row.raceKey).slice(0,8)))} ${esc(row.venue||"")} ${Number(row.raceNumber)||"?"}R</strong><span class="pill ${outcome==="hit"?"success":outcome==="miss"?"warning":""}">${outcomeLabel}</span></div>${metas([["発走",row.scheduledStartTime?formatTime(row.scheduledStartTime):"—"],["seal",row.predictionSealedAt?formatTime(row.predictionSealedAt):"—"],["予想品質",stateLabel.quality],["購入可否",stateLabel.purchase],["資金配分",stateLabel.funding],["MAIN / COVER",`${row.mainTicketCount||0} / ${row.coverTicketCount||0}`],["確定着順",result],["払戻",money(row.payout)],["100円flat投資",money(evaluation.investment)],["100円flat回収",money(evaluation.return)],["100円flat ROI",roi(evaluation.roi)]])}</button></article>`}
...
function savedLifecycle(r){return(state.collectorStatus?.races||[]).find(row=>row.raceKey===raceKey(r))||null}
