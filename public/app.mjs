...
const APP_RELEASE="KEIRIN-0.16.50-composite-purchase-border";
...
function renderPurchaseBorderAudit(audit){const data=audit?.purchaseBorderAudit;if(!data||!Array.isArray(data.rows)||!data.rows.length)return"";const adoptedBelow=Number(data.adoptedBelowBorderCount)||0;const rows=data.rows.filter(row=>!row.eligible||row.adopted).slice(0,60).map(row=>{const m=row.metrics||{};const status=row.adopted?"購入":"不採用";const why=Array.isArray(row.failures)&&row.failures.length?row.failures.join(" / "):"通過";return `<div class="abilityRow auditKeyValueRow"><strong>${esc(row.order)} ${esc(status)}</strong><span>終端 ${fmtPct(m.terminalRelative)} / 頭 ${fmtPct(m.familyRelative)} / 枝 ${fmtPct(m.branchRelative)} / 2着 ${fmtPct(m.secondRelative)} / 3着 ${fmtPct(m.thirdRelative)} / 条件負荷 ${fmtPct(m.burdenFactor)} / ${esc(why)}</span></div>`}).join("");return `<h3>買い目化ボーダー監査</h3><div class="auditCallout ${adoptedBelow?"danger":""}"><strong>${adoptedBelow?"ボーダー未達の通常購入あり":"ボーダー未達の通常購入なし"}</strong><p>通過 ${Number(data.eligibleCount)||0}件 / 未達 ${Number(data.rejectedCount)||0}件。終端・頭・枝・2着・3着の相対確率と累積差分条件負荷を、確率カバーより先に判定します。</p><p class="muted">初期研究値: 終端45% / 頭45% / 枝50% / 2着50% / 3着50%。終端75%以上かつ頭60%以上・枝70%以上・2着70%以上なら3着45%まで救済。高配当は別の妙味ゲートで評価します。</p></div>${rows?`<div class="abilityList auditKeyValueList">${rows}</div>`:""}`};
