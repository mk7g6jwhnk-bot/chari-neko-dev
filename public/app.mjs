...
const APP_RELEASE="KEIRIN-0.5.20-girls-evidence-gate";
...
async function loadCollectorStatus(){if(state.collectorStatusBusy)return;state.collectorStatusBusy=true;const cached=loadCachedCollectorStatus(localStorage);if(cached&&!state.collectorStatus){state.collectorStatus={...cached,statusReadFailed:true,localCache:true};renderCollectorStatus()}try{const p=await jsonFetch("/.netlify/functions/keirin-collector-status");state.collectorStatus=p;saveCachedCollectorStatus(localStorage,p);renderCollectorStatus()}catch(error){state.collectorStatus=cached?{...cached,statusReadFailed:true,localCache:true,statusReadError:error?.message||String(error)}:{ok:false,statusReadFailed:true,error:error?.message||String(error)};renderCollectorStatus()}finally{state.collectorStatusBusy=false}}
function renderCollectorStatus(){const root=$("collectorProgress"),p=state.collectorStatus;if(!root)return;if(!p){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill">確認中</span></div>';return}const cached=loadCachedCollectorStatus(localStorage),view=collectorStatusViewModel(p,cached);if(view.mode==="STATUS_UNAVAILABLE"){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill warning">状態を確認できません</span></div><p class="muted">保存済み情報を表示中です。自動収集の実体が停止したことを示す表示ではありません。</p>';return}const s=view.status,stale=view.mode.startsWith("STALE"),actualError=view.mode==="COLLECTOR_ERROR"||view.mode==="STALE_COLLECTOR_ERROR",confirmed=s.checkedAt||s.snapshotGeneratedAt||s.lastResultRunAt||s.lastPredictionRunAt,healthy=actualError?(stale?"前回確認時エラー":"自動収集エラー"):(stale?`正常稼働中（前回確認 ${confirmed?formatTime(confirmed):"時刻不明"}）`:"正常稼働中"),updated=s.lastSuccessfulCollectorUpdate||s.lastResultRunAt||s.lastPredictionRunAt,r=s.researchProgress||{},progressLabel=(done,value,target)=>done?"完了":`${value||0} / ${target}`,notice=stale?'<p class="muted">現在の状態取得に失敗。保存済み情報を表示中です。</p>':"";root.innerHTML=`<div class="sectionHead"><h2>自動収集</h2><span class="pill ${actualError?"warning":"success"}">${healthy}</span></div>${notice}${metas([["予想取得済み",`${s.sealedPredictionCount}R`],["結果取得済み",`${s.resultLoadedCount}R`],["比較済み",`${s.verifiedCount}R`],["結果待ち",`${s.pendingResultCount}R`],["再試行中",`${s.retryingCount}R`],["最終更新",updated?formatTime(updated):"未確認"]])}<h3>研究評価</h3>${metas([["50R監査",progressLabel(r.complete50,r.progress50,50)],["100R監査",progressLabel(r.complete100,r.progress100,100)],["Cohort",r.cohort||"未確認"],["Report",r.reportStatus||"未生成"]])}${s.lastError?`<details><summary>詳細</summary><p class="muted">${esc(typeof s.lastError==="string"?s.lastError:`${s.lastError.code||"ERROR"} / ${s.lastError.raceKey||"race不明"} / ${s.lastError.stage||"stage不明"}`)}</p></details>`:""}`}
