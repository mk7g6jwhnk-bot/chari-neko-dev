...
function renderCollectorStatus(){const root=$("collectorProgress"),p=state.collectorStatus;if(!root)return;if(!p){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill">確認中</span></div>';return}const cached=loadCachedCollectorStatus(localStorage),view=collectorStatusViewModel(p,cached);if(view.mode==="STATUS_UNAVAILABLE"){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill warning">状態を確認できません</span></div><p class="muted">現在値も保存済み情報もありません。件数は確認できません。</p>';return}const s=view.status,autoStale=view.autoMode.startsWith("STALE"),researchStale=view.researchMode==="STALE",actualError=view.autoMode==="COLLECTOR_ERROR"||view.autoMode==="STALE_COLLECTOR_ERROR",confirmed=s.autoObservedAt||s.lastResultRunAt||s.lastPredictionRunAt,healthy=view.autoMode==="UNAVAILABLE"?"自動収集：確認できません":actualError?(autoStale?"前回確認時エラー":"自動収集エラー"):(autoStale?`正常稼働中（前回確認 ${confirmed?formatTime(confirmed):"時刻不明"}）`:"正常稼働中"),updated=s.lastSuccessfulCollectorUpdate||s.lastResultRunAt||s.lastPredictionRunAt,r=s.researchProgress,metric=(value,suffix="R")=>value===null||value===undefined?"確認できません":`${value}${suffix}`,progress=(value,target)=>value===null||value===undefined?"確認できません":`${value} / ${target}`,notice=(autoStale||researchStale||view.autoMode==="UNAVAILABLE"||view.researchMode==="UNAVAILABLE")?'<p class="muted">取得できなかった項目は、同じsourceの保存済みsnapshotを表示しています。保存値もない項目は「確認できません」と表示します。</p>':"";root.innerHTML=`<div class="sectionHead"><h2>今日の自動収集</h2><span class="pill ${actualError||view.autoMode==="UNAVAILABLE"?"warning":"success"}">${healthy}</span></div>${notice}${metas([["予想取得済み",metric(s.sealedPredictionCount)],["結果取得済み",metric(s.resultLoadedCount)],["比較済み",metric(s.verifiedCount)],["結果待ち",metric(s.pendingResultCount)],["再試行中",metric(s.retryingCount)],["最終更新",updated?formatTime(updated):"確認できません"]])}<h3>累積研究評価${researchStale&&s.researchObservedAt?`（前回確認 ${formatTime(s.researchObservedAt)}）`:""}</h3>${metas([["比較可能",metric(r?.comparableTotal)],["50R監査",progress(r?.progress50,50)],["100R監査",progress(r?.progress100,100)],["Cohort",r?.cohort||"確認できません"],["Report",r?.reportStatus||"確認できません"]])}${s.lastError?`<details><summary>詳細</summary><p class="muted">${esc(typeof s.lastError==="string"?s.lastError:`${s.lastError.code||"ERROR"} / ${s.lastError.raceKey||"race不明"} / ${s.lastError.stage||"stage不明"}`)}</p></details>`:""}`}
