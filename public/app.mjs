...
function renderCollectorStatus(){const root=$("collectorProgress"),p=state.collectorStatus;if(!root)return;if(!p){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill">確認中</span></div>';return}if(!p.ok){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill warning">状態確認失敗</span></div><p class="muted">開催一覧は保存済みsnapshotから表示できます。</p>';return}const healthy=p.collectorHealthy?"正常稼働中":"要確認",updated=p.lastResultRunAt||p.lastPredictionRunAt;root.innerHTML=`<div class="sectionHead"><h2>自動収集</h2><span class="pill ${p.collectorHealthy?"success":"warning"}">${healthy}</span></div>${metas([["予想取得済み",`${p.sealedPredictionCount}R`],["結果取得済み",`${p.resultLoadedCount}R`],["比較済み",`${p.verifiedCount}R`],["未確定結果",`${p.pendingResultCount}R`],["取得失敗",`${p.failureCount}R（再試行中 ${p.retryingCount}R）`],["最終更新",updated?formatTime(updated):"未確認"]])}${p.lastError?`<details><summary>詳細</summary><p class="muted">${esc(typeof p.lastError==="string"?p.lastError:`${p.lastError.code||"ERROR"} / ${p.lastError.raceKey||"race不明"} / ${p.lastError.stage||"stage不明"}`)}</p></details>`:""}`}
