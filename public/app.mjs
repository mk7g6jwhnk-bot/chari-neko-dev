...
const APP_RELEASE="KEIRIN-0.16.51-race-concentration-purchase-border";
...
function renderPurchaseBorderAudit(audit){const data=audit?.purchaseBorderAudit;if(!data||!Array.isArray(data.rows)||!data.rows.length)return"";const adoptedBelow=Number(data.adoptedBelowBorderCount)||0;const rc=data.raceConcentration||{};const rcText=Number.isFinite(Number(rc.severity))?`展開1位 ${fmtPct(rc.topBranchShare)} / 1着集中 ${fmtPct(rc.topFamilyShare)} / 上位5終端 ${fmtPct(rc.top5Share)} / 分散度 ${fmtPct(rc.severity)}（${esc(rc.regime||"NORMAL")}）`:"全体集中度 未記録";const rows=data.rows.filter(row=>!row.eligible||row.adopted).slice(0,60).map(row=>{const m=row.metrics||{};const status=row.adopted?"購入":"不採用";const why=Array.isArray(row.failures)&&row.failures.length?row.failures.join(" / "):"通過";const t=m.adaptiveThresholds||{};const adaptive=m.raceConcentrationAnchor?"アンカー":"適応ボーダー";return `<div class="abilityRow auditKeyValueRow"><strong>${esc(row.order)} ${esc(status)}</strong><span>終端 ${fmtPct(m.terminalRelative)} / 頭 ${fmtPct(m.familyRelative)} / 枝 ${fmtPct(m.branchRelative)} / 2着 ${fmtPct(m.secondRelative)} / 3着 ${fmtPct(m.thirdRelative)} / 条件負荷 ${fmtPct(m.burdenFactor)} / ${esc(adaptive)} [${fmtPct(t.terminal)}・${fmtPct(t.family)}・${fmtPct(t.branch)}・${fmtPct(t.second)}・${fmtPct(t.third)}] / ${esc(why)}</span></div>`}).join("");return `<h3>買い目化ボーダー監査</h3><div class="auditCallout ${adoptedBelow?"danger":""}"><strong>${adoptedBelow?"ボーダー未達の通常購入あり":"ボーダー未達の通常購入なし"}</strong><p>${rcText}</p><p>通過 ${Number(data.eligibleCount)||0}件 / 未達 ${Number(data.rejectedCount)||0}件。v216ではレース全体が分散するほど、主要頭のアンカー以外の終端だけボーダーを段階的に引き上げます。</p><p class="muted">基準値: 終端45% / 頭45% / 枝50% / 2着50% / 3着50%。分散が強いほど数％上乗せ。各主要頭の最上位終端は全体分散補正の対象外です。高配当は別の妙味ゲートで評価します。</p></div>${rows?`<div class="abilityList auditKeyValueList">${rows}</div>`:""}`};
