...
const APP_RELEASE="KEIRIN-0.9.0-parent-state-node-engine";
...
function renderNodeStateAudit(audit){
  ...
  if(!data)return "";
  return `<details class="supportBranchAudit"><summary>1ノード1事象・親状態継承監査（${data.passed?"整合":"要修正"}）</summary><p>検査経路 ${Number(data.checkedPathCount)||0} / 違反 ${Number(data.violationCount)||0} / 親状態継承違反 ${Number(data.inheritanceViolationCount)||0} / 1ノード1事象違反 ${Number(data.oneNodeOneEventViolationCount)||0}</p><p class="muted">2着は1着成立状態を保持したまま、3着は1着・2着成立状態を保持したまま、残り各車を再評価します。</p></details>`;
}
...
function betClassLabel(category){return({MAIN:"本線",COVER:"押さえ",BUYABLE_HIGH:"買える高配当"})[category]||"購入候補"}
...
function renderPredictionDetail(snapshot){
  ...
  try{
    const selections=Array.isArray(snapshot?.betSelections)?snapshot.betSelections:[];
...
function renderRiderMarkNameTable(riderMarks,participantMap,chatMarkMap){
  ...
  return `<div class="riderMarkTable"><div class="riderMarkHead"><span>車番</span><span>選手名</span><span>アプリ</span><span>チャット</span></div>${rows.map(m=>{const p=participantMap?.get?.(Number(m.number))||{},c=chatMarkMap?.get?.(Number(m.number))||null;return `<div class="riderMarkLine"><span class="riderMarkNo">${esc(m.number)}</span><span class="riderMarkName">${esc(p?.name||'選手名未取得')}<small>1着${esc(m.firstMark||'？')}・2着${esc(m.secondMark||'？')}・3着${esc(m.thirdMark||'？')}</small></span><strong class="riderMarkMain">${esc(m.overallMark||'？')}</strong><strong class="riderMarkChat">${esc(c?.overallMark||'未取込')}</strong></div>`}).join('')}</div>${hasChat?'':`<p class="auditWarn">チャット印が保存されていません。v77以前に取り込んだ予想、または印が認識できない形式の予想は再取り込みしてください。</p>`}`;
}

function renderRiderMarkAudit(audit){
  const rows=Array.isArray(audit?.warnings)?audit.warnings:[];
  const explanations=Array.isArray(audit?.explanations)?audit.explanations:[];
  const status=audit?.status||"OK";
  const title=status==="WARN"?"要修正":status==="CHECK"?"要確認":"整合";
  const warnHtml=rows.length
    ?`<div class="auditWarning"><strong>印 → 着順評価 → 買い目 接続監査：${esc(title)} ${rows.length}件</strong>${rows.map(w=>`<p>${w.severity==="high"?"⚠ ":"△ "}${esc(w.message)}</p>`).join("")}<p class="muted">印を買い目へ強制的に合わせません。ズレた場合は、主展開・位置・自然収束のどこで順位が逆転したか直接根拠を要求します。</p></div>`
    :`<div class="notice success"><strong>印 → 着順評価 → 買い目 接続監査：大きな矛盾なし</strong><p>総合印・1着印・2着印・3着印と購入終端を独立に照合しています。</p></div>`;
  const explainHtml=explanations.length
    ?`<details class="supportBranchAudit"><summary>印と買い目がズレる理由を見る</summary>${explanations.slice(0,10).map(x=>`<p class="muted">${esc(x.message)}</p>`).join("")}</details>`
    :"";
