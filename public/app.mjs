import{createPerformancePage}from"./performance-page.mjs";
import{raceLifecycleView,japanClock}from"./race-lifecycle-view.mjs";
...
import{allocatePreviewStakes,deriveThickBets,purchaseDisplayState,purchaseEligibility}from"./purchase-funding.mjs";
import{validateReusableSeal}from"./sealed-prediction-client.mjs";
import{createSealedResultController}from"./sealed-result-client.mjs";
import{getSealedPredictionCache,putSealedPredictionCache,recordReadMetric}from"./sealed-prediction-cache.mjs";
import{collectorStatusViewModel,loadCachedCollectorStatus,saveCachedCollectorStatus}from"./collector-status-view.mjs";
import{buildBetStory}from"./bet-story.mjs";
import{cacheValidationStatus,loadCachedValidationStatus,validationStatusModel}from"./validation-status-view.mjs";
const $=id=>document.getElementById(id),screens=[...document.querySelectorAll(".screen")];
const state={screen:"home",history:[],date:localDate(),meeting:null,meetings:[],meetingTab:"today",race:null,payload:null,snapshot:null,legacySnapshot:null,retry:null,busy:false,sealedLookupBusy:false,sealedLookupRaceKey:null,savedSummaries:[],savedSummaryDate:null,savedSummaryBusy:false,historyRecords:[],historyBusy:false,collectorStatus:null,collectorStatusBusy:false,validationStatus:null,validationBusy:false,oddsBusyKey:null,bulkBusy:false,bulkDone:0,bulkTotal:0,screeningBusy:false,deepDiveBusy:false,deepDiveCurrentKeys:[]};
const sealedResultController=createSealedResultController({elementById:$,raceKey,metas,escapeHtml:esc}),loadSealedResult=(...args)=>sealedResultController.load(...args);
const performancePage=createPerformancePage({root:$("predictionPerformance"),storage:localStorage,fetchJson:jsonFetch,openRace:openSealedSummary});
...
const APP_RELEASE="KEIRIN-0.5.29-terminal-trace-audit";
...
$("todayKeirin").onclick=()=>openMeetings();$("homeBtn").onclick=$("brand").onclick=()=>goHome();$("headerBack").onclick=()=>goBack();$("venueSelectBtn").onclick=()=>openVenueSelection();$("raceSelectBtn").onclick=()=>openRaceSelection();$("reloadMeetings").onclick=()=>loadMeetings(true);$("raceDate").onchange=e=>{state.date=e.target.value;loadMeetings()};$("tabToday").onclick=()=>setMeetingTab("today");$("tabBattle").onclick=()=>setMeetingTab("battle");$("tabVenues").onclick=()=>setMeetingTab("venues");$("predictBtn").onclick=()=>handleDetailPrimary();$("retryDetail").onclick=()=>handleDetailSecondary();$("backToDetail").onclick=()=>{if(state.snapshot){openSavedDetail(state.snapshot);return}renderDetail();show("detail")};$("checkResult").onclick=()=>checkResult();$("retryBtn").onclick=()=>state.retry?.();$("errorHome").onclick=()=>goHome();
window.addEventListener("popstate",()=>{if(state.history.length){state.history.pop();show(state.history.pop()||"home",false)}});
$("openPerformance").onclick=$("meetingsPerformance").onclick=()=>{show("performance");performancePage.open()};$("refreshPerformance").onclick=()=>loadCollectorStatus();
$("openPredictionHistory").onclick=()=>openPredictionHistory();$("reloadHistory").onclick=()=>loadPredictionHistory();$("historyStatus").onchange=()=>renderPredictionHistory();$("historyVenue").oninput=()=>renderPredictionHistory();
$("openValidationStatus").onclick=()=>{show("validation");void loadValidationStatus()};
...
function isCurrentSnapshot(snapshot){return Boolean(snapshot&&snapshot.predictionVersion===SNAPSHOT_COMPATIBLE_VERSION)}
...
async function jsonFetch(url){const requestUrl=String(url).includes("/.netlify/functions/keirin-predict?")?`${url}&display=1`:url,r=await fetch(requestUrl,{cache:"no-store"});let p;try{p=await r.json()}catch{throw new Error(`サーバー応答を読み取れません（HTTP ${r.status}）`)}if(!r.ok||p.ok===false)throw new Error(p.error||`取得失敗（HTTP ${r.status}）`);return p}


function renderHomeRecommendations(){
  const root=$("todayRecommendations"),rows=(state.collectorStatus?.races||[]).map(row=>({date:row.date||String(row.raceKey).slice(0,8),venueCode:row.venueCode,venueName:row.venueName,raceNo:row.raceNumber,scheduledStart:row.scheduledStartTime,deadline:row.deadline,saved:row})).filter(r=>compact(r.date)===compact(localDate())&&raceLifecycleView(r,r.saved).phase==="pre-race").sort((a,b)=>parseTime(a.date,a.scheduledStart)-parseTime(b.date,b.scheduledStart));
  $("recommendationCount").textContent=`${rows.length}R`;
  const visible=rows.slice(0,5);
  root.innerHTML=visible.length?visible.map((r,i)=>`<button class="secondary" data-home-race="${i}"><strong>${esc(r.venueName)} ${r.raceNo}R</strong><br>発走 ${esc(japanClock(r.scheduledStart))} / 締切 ${r.deadline?esc(japanClock(r.deadline)):"詳細で確認"}</button>`).join(""):'<p class="muted">保存済みの発走前レースはありません。開催一覧から確認できます。</p>';
  root.querySelectorAll("[data-home-race]").forEach(button=>button.onclick=()=>openSealedSummary(visible[Number(button.dataset.homeRace)].saved));
  const venues=[...new Map(rows.map(r=>[r.venueCode,r])).values()];
  $("homeVenues").innerHTML=venues.length?venues.map(r=>`<button class="secondary" data-home-venue="${esc(r.venueCode)}">${esc(r.venueName||r.venueCode)}</button>`).join(""):'<p class="muted">開催情報は「競輪を開く」から確認できます。</p>';
  $("homeVenues").querySelectorAll("[data-home-venue]").forEach(button=>button.onclick=()=>openVenueSelection());
}
...
async function loadMeetings(force=false){const dateKey=compact(state.date),cached=meetingCacheFor(dateKey);if(cached?.meetings?.length&&!force){state.meetings=cached.meetings;$("meetingCount").textContent=`${cached.meetings.length}会場`;renderVenueGrid(state.meetings);renderMeetingTabs();updateBulkRefreshUi("保存済み開催一覧を表示中（更新待ち）。公式状態をバックグラウンド更新しています。");show("meetings");void refreshMeetingsFromServer(dateKey).catch(()=>updateBulkRefreshUi("保存済み一覧を表示中（開催情報は更新待ち）です。"));void loadCollectorStatus();return}setLoading("開催情報を取得中","保存済みの公式開催snapshotを確認しています。");try{await refreshMeetingsFromServer(dateKey);show("meetings");void collectFinishedResultOnlyResearch();void loadCollectorStatus()}catch(e){if(cached?.meetings?.length){state.meetings=cached.meetings;$("meetingCount").textContent=`${cached.meetings.length}会場`;renderVenueGrid(state.meetings);renderMeetingTabs();updateBulkRefreshUi("開催情報の再取得に失敗しました。保存済み一覧を表示中（更新待ち）です。");show("meetings");void loadCollectorStatus();return}fail("開催情報の取得失敗",new Error("開催取得サービスが一時的に停止しています。数秒後に再試行してください。"),()=>loadMeetings(true))}}
async function refreshMeetingsFromServer(dateKey){const started=performance.now(),response=await fetch(`/.netlify/functions/keirin-discover?date=${dateKey}`,{headers:{accept:"application/json"}}),text=await response.text(),networkMs=performance.now()-started,parseStarted=performance.now();let p=null;try{p=JSON.parse(text)}catch{throw new Error("JSON_PARSE_FAILED")};const parseMs=performance.now()-parseStarted;if(!response.ok||!p?.ok)throw new Error(p?.code||p?.error||`HTTP ${response.status}`);const items=(p.meetings||[]).filter(m=>getCard(m)).sort((a,b)=>Number(venueCode(a))-Number(venueCode(b))),renderStarted=performance.now();state.meetings=items;storeMeetingCache(dateKey,items,p.checkedAt);$("meetingCount").textContent=`${items.length}会場`;renderVenueGrid(items);renderMeetingTabs();updateBulkRefreshUi(p.stale||p.cacheStatus==="STALE_SAVED"?(p.warning||"保存済み開催一覧を表示中（更新待ち）です。"):p.cacheStatus==="HIT"?"公式開催snapshotを再利用しました。":"公式開催snapshotを更新しました。");recordReadMetric({type:"meeting-summary",date:dateKey,cache:p.cacheStatus||"MISS",stale:Boolean(p.stale),networkMs:Math.round(networkMs*100)/100,jsonParseMs:Math.round(parseMs*100)/100,renderMs:Math.round((performance.now()-renderStarted)*100)/100,responseBytes:new TextEncoder().encode(text).byteLength,totalMs:Math.round((performance.now()-started)*100)/100});return p}
async function loadCollectorStatus(){if(state.collectorStatusBusy)return;state.collectorStatusBusy=true;let cached=loadCachedCollectorStatus(localStorage);if(cached&&!state.collectorStatus){state.collectorStatus=cached;renderStatusSurfaces()}try{const p=await jsonFetch("/.netlify/functions/keirin-collector-status");state.collectorStatus=p;renderStatusSurfaces();saveCachedCollectorStatus(localStorage,p)}catch(error){state.collectorStatus=state.collectorStatus?{...state.collectorStatus,statusReadFailed:true,autoCurrent:false,researchCurrent:false,statusReadError:error?.message||String(error)}:{ok:false,statusReadFailed:true,error:error?.message||String(error)};renderStatusSurfaces()}finally{state.collectorStatusBusy=false}}
function renderStatusSurfaces(){renderCollectorStatus();renderAutoCollectionSummary();renderPredictionPerformance();renderHomePredictionStatus();renderHomeSystemStatus();renderHomeRecommendations();if(state.meetings.length)renderMeetingTabs()}
function renderAutoCollectionSummary(){const root=$("collectorProgress"),s=collectorStatusViewModel(state.collectorStatus,loadCachedCollectorStatus(localStorage)).status;if(!root||!s)return;const value=(number,suffix="")=>number===null||number===undefined||!Number.isFinite(Number(number))?"—":`${Number(number)}${suffix}`,block=document.createElement("div");block.className="autoCollectionSummary metaGrid";block.innerHTML=metas([["開催",value(s.meetingCount)],["対象",value(s.todayRaceCount,"R")],["prefetch",value(s.prefetchedRaceCount,"R")],["発走前seal",value(s.preRaceSealSucceeded??s.sealedPredictionCount,"R")],["seal失敗",value(s.preRaceSealFailed,"R")],["発走前未達",value(s.missedBeforeStart,"R")]]);root.querySelector(".autoCollectionSummary")?.remove();root.querySelector(".sectionHead")?.insertAdjacentElement("afterend",block)}
function renderPredictionPerformance(){performancePage.updateStatus(state.collectorStatus)}

function renderCollectorStatus(){const root=$("collectorProgress"),p=state.collectorStatus;if(!root)return;if(!p){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill">確認中</span></div>';return}const cached=loadCachedCollectorStatus(localStorage),view=collectorStatusViewModel(p,cached);if(view.mode==="STATUS_UNAVAILABLE"){root.innerHTML='<div class="sectionHead"><h2>自動収集</h2><span class="pill warning">状態を確認できません</span></div><p class="muted">現在値も保存済み情報もありません。件数は確認できません。</p>';return}const s=view.status,autoStale=view.autoMode.startsWith("STALE"),researchStale=view.researchMode==="STALE",storageBlocked=view.autoMode.includes("STORAGE_BLOCKED"),actualError=storageBlocked||view.autoMode==="COLLECTOR_ERROR"||view.autoMode==="STALE_COLLECTOR_ERROR",confirmed=s.autoObservedAt||s.lastResultRunAt||s.lastPredictionRunAt,healthy=view.autoMode==="UNAVAILABLE"?"自動収集：確認できません":storageBlocked?"自動収集：保存停止中":actualError?(autoStale?"前回確認時エラー":"自動収集エラー"):(autoStale?`正常稼働中（前回確認 ${confirmed?formatTime(confirmed):"時刻不明"}）`:"正常稼働中"),updated=s.lastSuccessfulCollectorUpdate||s.lastResultRunAt||s.lastPredictionRunAt,r=s.researchProgress,metric=(value,suffix="R")=>value===null||value===undefined?"確認できません":`${value}${suffix}`,progress=(value,target)=>value===null||value===undefined?"確認できません":`${value} / ${target}`,storageNotice=storageBlocked?'<p class="muted">ストレージ容量不足のため、新しい予想・結果・研究観測を保存できません。</p>':"",notice=(autoStale||researchStale||view.autoMode==="UNAVAILABLE"||view.researchMode==="UNAVAILABLE")?'<p class="muted">取得できなかった項目は、同じsourceの保存済みsnapshotを表示しています。保存値もない項目は「確認できません」と表示します。</p>':"";root.innerHTML=`<div class="sectionHead"><h2>今日の自動収集</h2><span class="pill ${actualError||view.autoMode==="UNAVAILABLE"?"warning":"success"}">${healthy}</span></div>${storageNotice}${notice}<div class="metaGrid">${metas([["予想取得済み",metric(s.sealedPredictionCount)],["failures",metric(s.failureCount)],["結果取得済み",metric(s.resultLoadedCount)],["比較済み",metric(s.verifiedCount)],["結果待ち",metric(s.pendingResultCount)],["再試行中",metric(s.retryingCount)],["最終更新",updated?formatTime(updated):"確認できません"]])}</div><h3>累積研究評価${researchStale&&s.researchObservedAt?`（前回確認 ${formatTime(s.researchObservedAt)}）`:""}</h3><div class="metaGrid">${metas([["比較可能",metric(r?.comparableTotal)],["50R監査",progress(r?.progress50,50)],["100R監査",progress(r?.progress100,100)],["Cohort",r?.cohort||"確認できません"],["Report",r?.reportStatus||"確認できません"]])}</div>${s.lastError?`<details><summary>詳細</summary><p class="muted">${esc(typeof s.lastError==="string"?s.lastError:`${s.lastError.code||"ERROR"} / ${s.lastError.raceKey||"race不明"} / ${s.lastError.stage||"stage不明"}`)}</p></details>`:""}`}

function renderHomePredictionStatus(){const root=$("homePredictionStatus"),s=state.collectorStatus;if(!root)return;const period=s?.purchasePerformance?.periods?.today,empty=period?.evaluatedRaces===0,pct=n=>Number.isFinite(Number(n))?`${(Number(n)*100).toFixed(1)}%`:"—";root.innerHTML=`<div class="sectionHead"><h2>今日の予想状況</h2><span class="pill">100円flat</span></div>${metas([["予想済み",s?.sealedPredictionCount==null?"—":`${s.sealedPredictionCount}R`],["結果待ち",s?.pendingResultCount==null?"—":`${s.pendingResultCount}R`],["的中",empty?"未集計":period?`${period.hitRaces} / ${period.betRaces}R`:"—"],["100円flat ROI",empty?"未集計":pct(period?.roi)]])}<p class="muted">${empty?"まだ集計対象レースがありません。購入判定を保存できたレースから集計します。":"実stake成績は保存されたstakeがある場合だけ別集計します。"}</p>`}
function renderHomeSystemStatus(){const root=$("homeSystemStatus"),raw=state.collectorStatus;if(!root)return;const view=collectorStatusViewModel(raw,loadCachedCollectorStatus(localStorage));if(!view.status){root.innerHTML='<div class="sectionHead"><h2>研究・システム状態</h2><span class="pill warning">状態を確認できません</span></div><p class="muted">現在値も保存済み値もありません。</p>';return}const s=view.status,r=s.researchProgress||{},stale=view.autoSourceLevel!=="current"||view.researchSourceLevel!=="current",source=view.autoSourceLevel==="server-last-known"?"最新取得失敗・保存済み値":view.autoSourceLevel==="browser-last-known"?"端末保存値":"現在値",updated=s.lastSuccessfulCollectorUpdate||s.autoObservedAt||s.researchObservedAt,age=updated?relativeAge(updated):"不明";root.innerHTML=`<div class="sectionHead"><h2>研究・システム状態</h2><span class="pill ${stale?"warning":"success"}">${stale?"stale":"current"}</span></div><div class="metaGrid">${metas([["Research compared",r.comparableTotal??"—"],["collector",s.collectorOperational===true?"稼働":s.collectorOperational===false?"停止":"不明"],["last update",updated?`${age}前`:"—"],["source",source],["errors",s.failureCount??"—"]])}</div><details><summary>運用詳細</summary><div class="metaGrid">${metas([["開催",s.meetingCount??"—"],["対象",s.todayRaceCount??"—"],["prefetch",s.prefetchedRaceCount??"—"],["seal",s.preRaceSealSucceeded??s.sealedPredictionCount??"—"],["queue",s.retryingCount??"—"],["storage",s.storageWritable===true?"writable":s.storageWritable===false?"blocked":"unknown"],["browser",s.browserConnected===true?"connected":s.browserConnected===false?"disconnected":"unknown"],["raw status",view.autoSourceLevel]])}</div></details>`}
function relativeAge(value){const ms=Date.now()-Date.parse(value);if(!Number.isFinite(ms)||ms<0)return"時刻不明";if(ms<60000)return`${Math.floor(ms/1000)}秒`;if(ms<3600000)return`${Math.floor(ms/60000)}分`;if(ms<86400000)return`${Math.floor(ms/3600000)}時間`;return`${Math.floor(ms/86400000)}日`}

function openPredictionHistory(){const to=localDate(),from=new Date(`${to}T00:00:00+09:00`);from.setDate(from.getDate()-6);$("historyTo").value=to;$("historyFrom").value=from.toISOString().slice(0,10);show("history");void loadPredictionHistory()}
async function loadValidationStatus(){if(state.validationBusy)return;state.validationBusy=true;const cached=loadCachedValidationStatus(localStorage);if(cached){state.validationStatus={...cached,source:"LAST_KNOWN"};renderValidationStatus()}try{const r=await fetch(`/.netlify/functions/keirin-validation-status?t=${Date.now()}`,{cache:"no-store",headers:{accept:"application/json"}});const value=await r.json();if(!r.ok||value.ok===false)throw Error(value.code||`HTTP_${r.status}`);state.validationStatus=value;cacheValidationStatus(localStorage,value)}catch(error){state.validationStatus=cached?{...cached,source:"LAST_KNOWN",fetchError:error.message}:{status:"UNAVAILABLE",error:error.message}}finally{state.validationBusy=false;renderValidationStatus()}}
function renderValidationStatus(){const model=validationStatusModel(state.validationStatus||{}),badge=$("validationStatusBadge"),label=model.statusTone==="ok"?"正常":model.statusTone==="warning"?"注意":model.statusTone==="failed"?"失敗":"確認不可",pct=n=>n!==null&&n!==undefined&&n!==""&&Number.isFinite(Number(n))?`${(Number(n)*100).toFixed(1)}%`:"—",num=n=>Number.isFinite(Number(n))?Number(n):0;badge.textContent=label;badge.className=`pill ${model.statusTone==="ok"?"success":model.statusTone==="unknown"?"":"warning"}`;$("validationOverview").innerHTML=`<div class="sectionHead"><h2>現在の検証状態</h2><span class="validationSignal ${model.statusTone}">${label}</span></div>${metas([["累積",`${model.total}R`],["今回追加",`${model.newRaces}R`],["Integrity",model.integrityOk?"異常なし":"警告あり"],["最終検証",model.lastValidatedAt?formatTime(model.lastValidatedAt):"確認できません"],["Status生成",model.generatedAt?formatTime(model.generatedAt):"確認できません"],["鮮度",`${model.freshness?.label||"不明"}${model.source==="LAST_KNOWN"?"（最終取得値）":""}`],["Checkpoint",model.checkpoint||"—"],["Scheduler",model.scheduler.status||"不明"]])}`;const d=model.daily,dist=d.distance?.summary||{},trio=d.trio?.summary||{},marks=d.riderMarks?.summary||{},all=d.basic?.all||{};$("validationDaily").innerHTML=`<div class="sectionHead"><h2>最新日次</h2><span class="pill">${model.newRaces}R</span></div>${metas([["的中",all.tickets==null?`${num(all.hits)}R`:`${num(all.hits)} / ${num(all.tickets)}点`],["ROI",pct(all.roi)],["内部候補 P3 / P2 / P1 / P0",`${num(dist.riderSelection?.P3)} / ${num(dist.riderSelection?.P2)} / ${num(dist.riderSelection?.P1)} / ${num(dist.riderSelection?.P0)}`],["三連複signal",`${num(trio.trifectaMissTrioHit)}R`],["評価印 勝者Top3",`${num(marks.winnerTop3)}R`],["Warning",model.integrityIssues.length?model.integrityIssues.map(x=>x.code).join(" / "):"なし"]])}`;const survival=d.distance?.terminalSurvival||{},sd=survival.daily||{},sc=survival.cumulative||{},st=sd.stages||{},dr=sd.drops||{},rs=sd.reasons||{},fc=sd.finalRiderCoverage||{};$("validationTerminalSurvivalBody").innerHTML=sd.races==null?`<p class="muted">次回の日次検証から集計します。</p>`:metas([["日次 生存",`${num(st.GENERATED)} → ${num(st.MEANINGFUL)} → ${num(st.PURCHASE_CANDIDATE)} → ${num(st.FINAL_PURCHASE)}`],["脱落 M / C / 購入",`${num(dr.DROP_AT_MEANINGFUL)} / ${num(dr.DROP_AT_PURCHASE_CANDIDATE)} / ${num(dr.DROP_AT_FINAL_PURCHASE)}`],["購入不可 / 的中",`${num(dr.PURCHASE_INELIGIBLE)} / ${num(dr.EXACT_PURCHASE_HIT)}`],["方向・順位 / 3着 / 圧縮",`${num(rs.PAIR_DIRECTION_OR_RANK)} / ${num(rs.THIRD_CONDITIONAL)} / ${num(rs.PURCHASE_COMPRESSION)}`],["最終購入の車券内 P3 / P2 / P1 / P0",`${num(fc.P3)} / ${num(fc.P2)} / ${num(fc.P1)} / ${num(fc.P0)}`],["累積 完全 / 部分復元",`${num(sc.reconstructed)} / ${num(sc.partiallyReconstructed)}R`],["復元不能 UNKNOWN",`${num(sc.unreconstructable)} / ${num(sc.races)}R`]])+`<p class="muted">生存順: 生成 → 意味あり → 購入候補 → 最終購入。内部候補P値と、最終購入車券に含まれた結果3車の数は別集計です。</p>`;const xd=d.exactTerminalDistance?.summary||{},xr=xd.rankCapture||{},xb=xd.boundaryCapture||{},xdb=xd.distanceBins||{},xab=xd.additionalTicketBins||{};$("validationExactDistanceBody").innerHTML=xd.races==null?`<p class="muted">次回の日次検証から集計します。</p>`:metas([["評価可能 / 部分 / UNKNOWN",`${num(xd.fullyEvaluable)} / ${num(xd.partiallyEvaluable)} / ${num(xd.unknown)}R`],["正解terminal Top3 / 5 / 10 / 20",`${pct(xr[3]?.rate)} / ${pct(xr[5]?.rate)} / ${pct(xr[10]?.rate)} / ${pct(xr[20]?.rate)}`],["meaningful境界 内 / +1 / +2 / +3",`${num(xdb.INSIDE)} / ${num(xdb['+1'])} / ${num(xdb['+2'])} / ${num(xdb['+3'])}`],["meaningful境界 +4〜5 / +6〜10 / +11以上",`${num(xdb['+4_TO_5'])} / ${num(xdb['+6_TO_10'])} / ${num(xdb['+11_TO_20'])+num(xdb['+21_PLUS'])}`],["境界+0 / +1 / +2 / +3 capture",`${pct(xb[0]?.rate)} / ${pct(xb[1]?.rate)} / ${pct(xb[2]?.rate)} / ${pct(xb[3]?.rate)}`],["正解到達 追加+0 / +1 / +2 / +3",`${num(xab['+0'])} / ${num(xab['+1'])} / ${num(xab['+2'])} / ${num(xab['+3'])}`],["追加+4〜5 / +6以上",`${num(xab['+4_TO_5'])} / ${num(xab['+6_TO_10'])+num(xab['+11_PLUS'])}`]])+`<p class="muted">Research観測専用。結果からpredictionを再生成せず、保存済みterminal内の位置だけを表示します。</p>`;const tt=d.terminalTrace||{};$("validationTerminalTraceBody").innerHTML=tt.target==null?`<p class="muted">新方式の新規レースから観測します。</p>`:metas([["監査状態",tt.traceAuditStatus||"TRACE_AUDIT_OK"],["完全 / 部分",`${num(tt.complete)} / ${num(tt.partial)}R`],["不正 / 保存失敗",`${num(tt.invalid)} / ${num(tt.saveFailed)}R`],["score / rank不整合",`${num(tt.scoreMismatch)} / ${num(tt.rankMismatch)}`],["evidence重複疑い",`${num(tt.duplicateEvidenceSuspicion)}`],["hash / 結果混入 / SHADOW混入",`${num(tt.hashMismatch)} / ${num(tt.resultLeakage)} / ${num(tt.shadowContamination)}`],["保存 / 対象",`${num(tt.saved)} / ${num(tt.target)}`],["寄与診断可能 / UNKNOWN",`${num(tt.diagnosable)} / ${num(tt.unknown)}`]])+`<p class="muted">詳細異常は日次artifactに保存し、この画面には件数だけを表示します。production予想には接続しません。</p>`;const sh=d.shadowLab||{},shRows=Array.isArray(sh.variants)?sh.variants:[];$(`validationShadowLabBody`).innerHTML=shRows.length?`<div class="shadowVariantList">${shRows.map(row=>`<div class="shadowVariantRow"><strong>${esc(row.parameterName)} / ${esc(row.variantId)}<small>${esc(row.state||"DATA_NOT_ENOUGH")}</small></strong><span>${num(row.races)}R / 生存 ${num(row.exactSurvival)} / 的中 ${pct(row.hitRate)} / ROI ${pct(row.roi)} / 平均 ${row.avgTickets!==null&&row.avgTickets!==undefined&&row.avgTickets!==""&&Number.isFinite(Number(row.avgTickets))?Number(row.avgTickets).toFixed(1):`—`}点</span></div>`).join("")}</div><p class="muted">Research専用。production予想・購入・おすすめ・THICKは変更しません。protected final使用 ${num(sh.protectedFinalUsed)}R / hash mismatch ${num(sh.productionHashMismatch)}。</p>`:`<p class="muted">再生可能な保存traceがある新規日次raceから観測します。</p>`;const m=model.milestones;$("validationMilestones").innerHTML=`<div class="sectionHead"><h2>節目</h2><span class="pill">自動調整なし</span></div>${metas([["到達済み観測節目",m.reached?`${m.reached}R`:"未到達"],["次の観測節目",`${m.next}R`],["到達済み調整判断",m.adjustmentReached.length?m.adjustmentReached.map(x=>`${x}R`).join(" / "):"なし"],["次の調整判断",m.nextAdjustment?`${m.nextAdjustment}R`:"完了"]])}`;$("validationDetailBody").innerHTML=metas([["Status",model.status],["Prediction mismatch",d.integrity?.predictionMismatch??"—"],["Purchase mismatch",d.integrity?.purchaseMismatch??"—"],["Result mismatch",d.integrity?.sealedResultMismatch??"—"],["Catch-up",(model.scheduler.catchUpEnabled??model.scheduler.startWhenAvailable)?"有効":"不明"],["多重実行",model.scheduler.multipleInstances||"不明"],["Report",model.reportPath||"—"]])+(model.reportPath?`<p><a class="validationReportLink" href="${esc(model.reportPath)}" target="_blank" rel="noopener">最終reportを開く</a></p>`:"");renderValidationStructures(model.structure)}
function renderValidationStructures(structure){const root=$("validationStructureBody"),s=structure?.summary;if(!s){root.innerHTML='<p class="muted">構造観測はまだありません。</p>';return}const section=(title,rows)=>`<h3>${title}</h3><div class="structureCards">${(rows||[]).slice(0,8).map(r=>{const rate=r.P2PlusRate??(r.races?(Number(r.P3||0)+Number(r.P2||0))/r.races:null),sample=r.sampleStatus==="DATA_NOT_ENOUGH"?" / 標本不足":"";return`<article><strong>${esc(String(r.key))}</strong><small>${r.races}R / P2+ ${pctInline(rate)}${sample}</small>${r.hits==null?`<small>pair miss ${r.pairDirectionMiss??"—"} / third miss ${r.thirdConditionalMiss??"—"}</small>`:`<small>的中 ${r.hits}R / ROI ${pctInline(r.roi)}</small>`}</article>`}).join("")||'<p class="muted">データなし</p>'}</div>`;const overview=s.available==null?"":metas([["利用可能",`${s.available}R`],["UNKNOWN",`${s.unknown}R`],["ガールズ別扱い",`${s.girls}R`]]);root.innerHTML=`${overview}${section("車立て",s.byFieldSize)}${section("分線",s.byLineCount)}${section("単騎",s.bySoloCount)}${section("ライン構成",s.byLineSizePattern)}<p class="muted">構造はResearch観測専用です。おすすめ・見送り・購入・選手scoreには接続していません。</p>`}
function pctInline(value){return Number.isFinite(Number(value))?`${(Number(value)*100).toFixed(0)}%`:"—"}
async function loadPredictionHistory({homeOnly=false}={}){if(state.historyBusy)return;state.historyBusy=true;const to=compact(homeOnly?localDate():$("historyTo").value||localDate()),from=compact(homeOnly?(()=>{const d=new Date();d.setDate(d.getDate()-6);return localDateOf(d)})():$("historyFrom").value||localDate());if(!homeOnly)$("historyNotice").textContent="Railwayの保存済み予想を読み取り中…";try{const data=await jsonFetch(`/.netlify/functions/keirin-prediction-history?from=${from}&to=${to}`);state.historyRecords=Array.isArray(data.records)?data.records:[];renderPredictionHistory();renderHomeRecentResults();renderHomeHighlights();if(!homeOnly)$("historyNotice").textContent=data.partial?`一部の日付を取得できませんでした（${data.errors.length}日）。取得済みデータを表示します。`:`${data.from}〜${data.to} / ${data.count}件`;}catch(error){if(!homeOnly)$("historyNotice").textContent=`履歴を取得できません: ${error.message}`}finally{state.historyBusy=false}}
function localDateOf(date){const z=new Date(date.getTime()-date.getTimezoneOffset()*60000);return z.toISOString().slice(0,10)}
function canonicalHistoryPurchase(row){const e=row?.purchaseEvaluation||{},allowed=row?.canonicalEligibility?.isPurchaseTarget===true&&e.purchaseEligibility==="PURCHASE_ALLOWED";return{allowed,exclusionReason:row?.canonicalEligibility?.exclusionReason||e.exclusionReason||"LEGACY_UNKNOWN",hit:allowed&&e.standardHit===true,investment:allowed?Number(e.investment)||0:0,return:allowed?Number(e.return)||0:0,roi:allowed&&Number.isFinite(Number(e.roi))?Number(e.roi):null}}
function historyOutcome(row){if(!row.resultAttached)return"waiting";const e=canonicalHistoryPurchase(row);if(!e.allowed)return"blocked";return e.hit?"hit":"miss"}
function historyState(row){const quality=row.noBet?"低":row.judgement==="購入可"?"高":"中",purchase=row.purchaseAllowed?"購入可":"購入不可",funding=row.purchaseAllowed&&row.thick?"厚め候補":row.purchaseAllowed?"通常":"なし";return{quality,purchase,funding,label:purchase==="購入不可"?"見送り":quality==="高"?"購入可":"注意"}}
function filterHistory(){const status=$("historyStatus")?.value||"all",venue=String($("historyVenue")?.value||"").trim();return state.historyRecords.filter(row=>!venue||String(row.venue||"").includes(venue)).filter(row=>status==="all"||status===historyOutcome(row)||status==="allowed"&&row.purchaseAllowed||status==="caution"&&historyState(row).label==="注意"||status==="blocked"&&!row.purchaseAllowed)}
function renderPredictionHistory(){const root=$("historyList");if(!root)return;const rows=filterHistory();$("historyCount").textContent=`${rows.length}件`;root.innerHTML=rows.length?rows.map((row,index)=>historyCard(row,index)).join(""):'<section class="card empty">該当する保存済み予想はありません。</section>';root.querySelectorAll("[data-history]").forEach(button=>button.onclick=()=>openSealedSummary(rows[Number(button.dataset.history)]))}
function historyCard(row,index){const stateLabel=historyState(row),evaluation=canonicalHistoryPurchase(row),result=(row.finishOrder||[]).join("-")||"結果待ち",outcome=historyOutcome(row),outcomeLabel={hit:"的中",miss:"不的中",waiting:"結果待ち",blocked:"購入成績対象外"}[outcome],money=value=>value==null?"—":`${Number(value).toLocaleString()}円`,roi=value=>Number.isFinite(Number(value))?`${(Number(value)*100).toFixed(1)}%`:"—",reason=outcome==="blocked"?purchaseExclusionReasonText(evaluation.exclusionReason):"—";return `<article class="card compact historyCard"><button class="historyOpen" data-history="${index}"><div class="sectionHead"><strong>${esc(formatDate(String(row.raceKey).slice(0,8)))} ${esc(row.venue||"")} ${Number(row.raceNumber)||"?"}R</strong><span class="pill ${outcome==="hit"?"success":outcome==="miss"?"warning":""}">${outcomeLabel}</span></div>${metas([["発走",row.scheduledStartTime?formatTime(row.scheduledStartTime):"—"],["seal",row.predictionSealedAt?formatTime(row.predictionSealedAt):"—"],["予想品質",stateLabel.quality],["購入可否",evaluation.allowed?"購入可":"購入不可"],["購入対象外理由",reason],["資金配分",evaluation.allowed?stateLabel.funding:"なし"],["MAIN / COVER",evaluation.allowed?`${row.mainTicketCount||0} / ${row.coverTicketCount||0}`:"0 / 0"],["確定着順",result],["公式3連単配当",money(row.payout)],["購入投資",evaluation.allowed?money(evaluation.investment):"—"],["購入払戻",evaluation.allowed?money(evaluation.return):"—"],["購入ROI",evaluation.allowed?roi(evaluation.roi):"—"]])}</button></article>`}
function purchaseExclusionReasonText(code){return({PURCHASE_INELIGIBLE:"購入不可判定",NO_CANONICAL_PURCHASE_PLAN:"canonical買い目なし",NO_RESULT:"結果未確定",NO_PAYOUT:"払戻未確定",VOID:"レース不成立",REFUND:"返還",ABNORMAL_EXCLUDED:"integrity対象外",LEGACY_UNKNOWN:"旧記録・購入資格不明",BROKEN_SNAPSHOT:"購入snapshot不完全"})[code]||String(code||"購入対象外")}
function renderHomeRecentResults(){const root=$("homeRecentResults");if(!root)return;const rows=state.historyRecords.filter(row=>row.resultAttached).slice(0,5);root.innerHTML=`<div class="sectionHead"><h2>最近の結果</h2><button id="openPredictionHistory" class="secondary" type="button">予想履歴を開く</button></div>${rows.length?rows.map((row,index)=>`<button class="historyMini" data-recent="${index}"><strong>${esc(row.venue||"")} ${row.raceNumber}R</strong><span>${(row.finishOrder||[]).join("-")||"—"} / ${{hit:"的中",miss:"不的中",blocked:"評価対象外"}[historyOutcome(row)]||"結果待ち"}</span></button>`).join(""):'<p class="muted">直近7日に確定結果はありません。</p>'}`;root.querySelector("#openPredictionHistory").onclick=()=>openPredictionHistory();root.querySelectorAll("[data-recent]").forEach(button=>button.onclick=()=>openSealedSummary(rows[Number(button.dataset.recent)]))}
function renderHomeHighlights(){const root=$("homeHighlights");if(!root)return;const rows=state.historyRecords.filter(row=>!row.resultAttached),allowed=rows.filter(row=>row.purchaseAllowed).length,caution=rows.filter(row=>historyState(row).label==="注意").length,thick=rows.filter(row=>row.purchaseAllowed&&row.thick).length;root.innerHTML=`<div class="sectionHead"><h2>注目レース</h2><span class="pill">${rows.length}R</span></div>${metas([["購入可",allowed],["注意",caution],["厚め候補",thick],["荒れ気配","判定データ待ち"]])}`}
function renderVenueGrid(items){$("meetingList").className="venueGrid";$("meetingList").innerHTML=items.length?items.map((m,i)=>{const nums=raceNumbersOf(m),count=nums.length||12,range=nums.length?`${nums[0]}R〜${nums[nums.length-1]}R`:`${count}R`,next=meetingNextDeadline(m),band=meetingTimeBand(m),autoCount=(m.races||[]).filter(r=>r.autoStatus&&r.autoStatus!=="NOT_COLLECTED").length;return `<button class="venueCard venue-${band.key}" data-meeting="${i}" aria-label="${esc(m.venueName)}のレースを見る"><span class="venueCode">${esc(venueCode(m))}</span><span class="venueBand">${band.label}</span><strong>${esc(m.venueName)}</strong><small>${range}・${count}レース</small><span class="venueDeadline">${esc(next)}</span><span class="venueState ${m.allResultsConfirmed?"ready":"waiting"}">${autoCount?`自動状態 ${autoCount}R`:"状態未確認"}</span></button>`}).join(""):'<section class="card empty venueEmpty">この日の開催は見つかりませんでした。</section>';$("meetingList").querySelectorAll("[data-meeting]").forEach(b=>b.onclick=()=>openRaces(items[Number(b.dataset.meeting)]))}
...
function openRaces(meeting){state.meeting=meeting;$("venueTitle").textContent=meeting.venueName;$("raceDateLabel").textContent=formatDate(compact(state.date));const nums=raceNumbersOf(meeting),races=(nums.length?nums:Array.from({length:12},(_,i)=>i+1)).map(n=>raceFrom(meeting,n));$("raceCount").textContent=`${races.length}R`;$("raceList").innerHTML=races.map((r,i)=>{const s=raceStatus(r),battle=isBattleRace(r),deadline=deadlineOf(r),saved=displaySnapshotForRace(r),ended=s.label==="終了",action=ended?(saved?"結果・詳細 ›":"結果を見る ›"):(saved?"詳細を見る ›":"予想する ›");return `<article class="raceCardWrap"><button class="raceCard compactRace" data-race="${i}"><div class="raceTop"><h2>${r.raceNo}R</h2><span class="status ${s.className}">${s.label}</span></div><p class="raceDeadline">${deadline?`${r.deadline?"締切":"発走"} ${esc(deadline)}`:"締切確認中"}${saved?" ・ 保存済み":""}</p><span class="raceAction">${action}</span></button><button class="raceBattle ${battle?"active":""}" data-race-battle="${i}" aria-label="勝負レース${battle?"解除":"登録"}">${battle?"★":"☆"}</button></article>`}).join("");$("raceList").querySelectorAll("[data-race]").forEach(b=>b.onclick=()=>openDetail(races[Number(b.dataset.race)]));$("raceList").querySelectorAll("[data-race-battle]").forEach(b=>b.onclick=e=>{e.stopPropagation();toggleBattleRace(races[Number(b.dataset.raceBattle)]);openRaces(meeting)});show("races")}
function raceFrom(m,raceNo){const base={date:compact(state.date),venueCode:venueCode(m),venueName:m.venueName,raceNo},saved=findLatestSnapshot(localStorage,base),meta=raceMetaOf(m,raceNo)||{},cached=raceMetaCacheFor(base)||{};return{...base,scheduledStart:japanClock(savedLifecycle(base)?.scheduledStartTime)||meta.scheduledStart||meta.startTime||cached.startTime||cached.scheduledStart||saved?.targetRace?.scheduledStart||"",deadline:cached.deadline||cached.cutoffTime||meta.deadline||meta.cutoffTime||saved?.targetRace?.deadline||"",officialStatus:meta.officialStatus||"OFFICIAL_STATUS_UNKNOWN",resultStatus:meta.resultStatus||"OFFICIAL_STATUS_UNKNOWN",autoStatus:meta.autoStatus||"NOT_COLLECTED",predictionSealed:Boolean(meta.predictionSealed),resultConfirmed:Boolean(meta.resultConfirmed),compared:Boolean(meta.compared),raceCardUrl:getCard(m)?.url||"",oddsUrl:getOdds(m)?.url||""}}
...
function openDetail(race){state.race={...race};state.payload=null;state.snapshot=displaySnapshotForRace(race);state.legacySnapshot=state.snapshot&&isCurrentSnapshot(state.snapshot)?null:legacySnapshotForRace(race);if(raceStatus(race).label!=="終了"&&!isCurrentSnapshot(state.snapshot)){state.legacySnapshot=state.snapshot||legacySnapshotForRace(race);state.snapshot=null}renderDetail();show("detail");if(!state.snapshot)void reuseSealedPrediction(race);else if(state.snapshot?.sealedPrediction?.predictionHash)void loadSealedResult(state.snapshot)}
async function reuseSealedPrediction(race,expectedHash=null){const key=raceKey(race);if(state.sealedLookupBusy||state.snapshot)return;state.sealedLookupBusy=true;state.sealedLookupRaceKey=key;renderDetail();try{const lookupStarted=performance.now();let data=null,cacheHit=false,networkMs=0,parseMs=0,responseBytes=0,serverReadMs=null;if(expectedHash){try{const cached=await getSealedPredictionCache(key,expectedHash);if(cached?.payload?.predictionHash===expectedHash){data=cached.payload;cacheHit=true}}catch{}}if(!data){const networkStarted=performance.now(),response=await fetch(`/.netlify/functions/keirin-saved-prediction-detail?raceKey=${encodeURIComponent(canonicalReadRaceKey(race))}`,{cache:"no-store",headers:{accept:"application/json"}}),text=await response.text();networkMs=performance.now()-networkStarted;responseBytes=new TextEncoder().encode(text).byteLength;serverReadMs=Number(data?.serverReadMs||response.headers.get("server-timing")?.match(/dur=([\d.]+)/)?.[1])||null;if(response.status===404)return;const parseStarted=performance.now();try{data=JSON.parse(text)}catch{}parseMs=performance.now()-parseStarted;if(!response.ok||!data)return;if(expectedHash&&data.predictionHash!==expectedHash)return;try{await putSealedPredictionCache(key,data.predictionHash,data)}catch{}}const audit=validateReusableSeal(data,race,expectedHash&&race.resultConfirmed?data.predictionVersion:SNAPSHOT_COMPATIBLE_VERSION);if(!audit.passed)return;if(state.sealedLookupRaceKey!==key||raceKey(state.race)!==key||state.snapshot)return;const snapshot=createSnapshot(data.predictionPayload,new Date(data.predictionSealedAt));snapshot.sealedPrediction={source:data.source,predictionHash:data.predictionHash,inputHash:data.inputHash||null,predictionSealedAt:data.predictionSealedAt,integrityStatus:data.integrityStatus,temporalStatus:data.temporalStatus,immutable:true};state.payload=data.predictionPayload;state.snapshot=saveSnapshot(localStorage,snapshot);state.race={...state.race,...snapshot.targetRace};const renderStarted=performance.now();renderOfficial(state.payload);renderDetail();const renderMs=performance.now()-renderStarted;recordReadMetric({type:"prediction-detail",raceKey:key,predictionHash:data.predictionHash,cache:cacheHit?"hit":"miss",responseBytes,serverReadMs,networkMs:Math.round(networkMs*100)/100,jsonParseMs:Math.round(parseMs*100)/100,renderMs:Math.round(renderMs*100)/100,totalMs:Math.round((performance.now()-lookupStarted)*100)/100});renderHomeRecommendations();void loadSealedResult(state.snapshot)}catch{}finally{if(state.sealedLookupRaceKey===key){state.sealedLookupBusy=false;renderDetail()}}}
function openSealedSummary(summary){const parts=String(summary.raceKey||"").split("-");state.race={date:parts[0],venueCode:parts[1],raceNo:Number(parts[2]),venueName:summary.venue||summary.venueName||"",venue:summary.venue||summary.venueName||"",scheduledStart:summary.scheduledStartTime,resultConfirmed:summary.resultAttached===true||Boolean(summary.resultObservedAt),resultLifecycleState:summary.resultLifecycleState,lifecycleStatus:summary.lifecycleStatus};state.payload=null;state.snapshot=null;state.legacySnapshot=null;renderDetail();show("detail");void reuseSealedPrediction(state.race,summary.predictionHash)}
function openSavedDetail(snapshot){if(!snapshot)return;state.payload=null;state.snapshot=snapshot;const target=snapshot?.targetRace&&typeof snapshot.targetRace==="object"?snapshot.targetRace:{};state.race={...(state.race||{}),...target};try{renderDetail();show("detail");if(snapshot?.sealedPrediction?.predictionHash)void loadSealedResult(snapshot)}catch(error){console.error("saved detail render failed",error);renderBasicDetailFallback(snapshot,error);show("detail")}}
function renderDetail(){const r=state.race,s=raceStatus(r),cachedResult=resultCacheFor(r),resultFinal=Boolean(cachedResult&&!isResultPending(cachedResult)),ended=s.label==="終了"||resultFinal,timeKnown=Boolean(deadlineOf(r)),displayStatus=ended?{label:"終了",className:"danger"}:s;$("detailDate").textContent=formatDate(r.date);$("detailTitle").textContent=`${r.venueName} ${r.raceNo}R`;$("raceStatus").textContent=displayStatus.label;$("raceStatus").className=`pill ${displayStatus.className}`;$("raceMeta").innerHTML=metas([["日付",formatDate(r.date)],["会場",r.venueName],["レース",`${r.raceNo}R`],["締切",deadlineOf(r)||"詳細取得で確認"]]);renderOfficial(state.payload);renderPredictionDetail(state.snapshot);renderChatPredictionImport();renderDetailResult(state.snapshot?.result||cachedResult);if(state.snapshot){$("savedPrediction").classList.remove("hidden");$("savedPrediction").innerHTML=`<div class="sectionHead"><h2>保存済み予想あり</h2><span class="pill success">${formatTime(state.snapshot.createdAt)}</span></div><p class="muted">買い目・展開根拠は保存済みです。再生成しなくても確認できます。</p><button id="openSaved" class="secondary">保存済み買い目を見る</button>`;$("openSaved").onclick=()=>renderPrediction(state.snapshot)}else if(state.legacySnapshot&&!ended){$("savedPrediction").classList.remove("hidden");$("savedPrediction").innerHTML=`<div class="sectionHead"><h2>旧版の保存予想あり</h2><span class="pill warning">旧版</span></div><p class="muted">${esc(state.legacySnapshot.predictionVersion||"旧版")} の予想履歴は検証用に保持しています。未発走レースでは現行版の予想として使いません。</p><button id="openSaved" class="secondary">旧版履歴を見る</button>`;$("openSaved").onclick=()=>renderPrediction(state.legacySnapshot)}else $("savedPrediction").classList.add("hidden");$("predictBtn").disabled=false;if(ended){$("predictBtn").textContent=cachedResult||state.snapshot?.result?"公式結果を更新":"公式結果を見る";$("retryDetail").classList.toggle("hidden",!state.snapshot);if(state.snapshot)$("retryDetail").textContent="保存済み予想を見る"}else if(state.snapshot){$("predictBtn").textContent="保存済み買い目を見る";$("retryDetail").classList.remove("hidden");$("retryDetail").textContent="再予想"}else if(!timeKnown){$("predictBtn").textContent=cachedResult?"公式結果を更新":"公式結果を見る";$("retryDetail").classList.remove("hidden");$("retryDetail").textContent="このレースを予想"}else{$("predictBtn").textContent="このレースを予想";$("retryDetail").classList.add("hidden")}}
function handleDetailPrimary(){if(!state.race||state.sealedLookupBusy)return;const cachedResult=resultCacheFor(state.race),ended=raceStatus(state.race).label==="終了"||Boolean(cachedResult&&!isResultPending(cachedResult)),timeUnknown=!deadlineOf(state.race);if(ended||(!state.snapshot&&timeUnknown)){checkDetailResult();return}if(state.snapshot){renderPrediction(state.snapshot);return}predict()}
...
function renderBetGroups(snapshot){
  const groups=[["本線シナリオ群","MAIN"],["押さえシナリオ群","COVER"],["高配当属性","BUYABLE_HIGH"]],bets=standardSelections(snapshot),refs=referenceSelections(snapshot),panel=$("purchaseControls"),budget=Number(panel?.dataset.budget)||bets.reduce((s,b)=>s+(Number(b.stake)||0),0),mode=panel?.dataset.mode||"standard",stakes=allocatePreviewStakes(bets,budget,mode),thickKeys=new Set(deriveThickBets(snapshot).map(x=>x.order.join("-")));
  const groupHtml=groups.map(([label,key])=>{
    const rows=bets.map((b,i)=>({b,i})).filter(x=>x.b.category===key);
    if(rows.length){const families=new Map();for(const row of rows){const id=row.b.originatingScenarioFamily||row.b.scenarioFamilyLabel||row.b.branchLabel||"UNRESOLVED";if(!families.has(id))families.set(id,[]);families.get(id).push(row)}const familyHtml=[...families.entries()].map(([,familyRows])=>{const first=familyRows[0].b,familyLabel=first.scenarioFamilyLabel||first.branchLabel||"展開未確認";return `<div class="scenarioFamilyGroup"><h4>${esc(familyLabel)}</h4><div class="betRows">${familyRows.map(({b,i})=>betStoryHtml(snapshot,b,{thick:thickKeys.has(b.order.join("-")),stake:stakes?stakes[i]:null})).join("")}</div></div>`}).join("");return `<section class="betCard"><h3>${label}</h3>${familyHtml}</section>`}
  ...
  const referenceHtml=refs.length?`<section class="betCard"><h3>参考買い目</h3><div class="betRows">${refs.map(b=>betStoryHtml(snapshot,b,{reference:true})).join("")}</div><p class="muted">REFERENCEは標準購入ではありません。購入点数・資金配分・購入的中率から除外します。</p></section>`:"";
...
function betStoryHtml(snapshot,bet,{thick=false,stake=null,reference=false}={}){const story=buildBetStory(snapshot,bet),v=story.internalEvaluation,pct=v=>Number.isFinite(Number(v))?`${(Number(v)*100).toFixed(1)}%`:"未確認",price=reference?`${bet.odds?`${Number(bet.odds)}倍 / `:""}参考・資金配分対象外`:`${bet.odds?`${Number(bet.odds)}倍`:"オッズ確認待ち"}${stake!==null?` / ${Number(stake).toLocaleString()}円`:" / 配分不可"}`;return `<details class="betStory"><summary><span><strong>${esc(story.ticket)}${thick?"　🔥厚め":""}</strong><small>${esc(story.classification)}</small><em>${esc(story.brief)}</em></span><b>${esc(price)}</b></summary><div class="betStoryBody"><h5>【展開】</h5><p>${esc(story.scenario)}</p><h5>【なぜこの着順になるか】</h5><p>${esc(story.orderReason)}</p><h5>【崩れる条件】</h5><ul>${story.failureConditions.map(row=>`<li>${esc(row)}</li>`).join("")}</ul><h5>【内部評価】</h5><div class="betStoryMetrics"><span>${esc(v.classification)}</span><span>自然収束 ${esc(v.naturalConvergence)}${v.naturalConvergenceScore===null?"":` / ${pct(v.naturalConvergenceScore)}`}</span><span>シナリオ支持 ${esc(v.scenarioSupport)}${v.scenarioSupportScore===null?"":` / ${pct(v.scenarioSupportScore)}`}</span><span>モデル評価 ${pct(v.modelProbability)}</span><span>family ${esc(v.family||"未確認")} / ${pct(v.familyProbability)}</span><span>terminal ${pct(v.terminalProbability)}</span></div></div></details>`}
...
function scenarioBetSentence(b,explanation=null){
  ...
  if(b?.category==="MAIN"){
    purchaseSentence=`「${branch}」から直接つながる自然終端として${a}-${c}-${d}を本線にしました。`;
  }else if(b?.category==="COVER"){
    purchaseSentence=`${a}-${c}-${d}は「${branch}」から成立するものの、本線より追加条件があるため押さえにしました。`;
  }else{
    purchaseSentence=`${a}-${c}-${d}は「${branch}」由来の別展開として成立し、自然さだけで本線には上げず、オッズ妙味が残るため高配当候補にしました。`;
  }

  // The primary explanation is ONE causal narrative produced from the prediction branch.
  // Position-specific conditions remain available only as audit details; they are not
  // presented as the scenario itself.
  const scenarioText=String(scenario?.timeline||"").trim();
  const integrity=scenario?.scenarioIntegrity||null;
  const integrityOk=Boolean(integrity?.branchId && String(integrity.branchId)===String(b?.dominantBranchId)
    && Array.isArray(integrity.terminalOrder)
    && integrity.terminalOrder.length>=3
    && integrity.terminalOrder.slice(0,3).map(Number).join("-")===order.slice(0,3).join("-"));
  const linkedText=integrityOk&&scenarioText
    ?scenarioText
    :`保存済みの予測枝「${branch}」から${a}-${c}-${d}へつながる終端シナリオを確認できました。`;

  const scenarioReasons=Array.isArray(scenario?.reasons)?scenario.reasons:[];
  const auditReasons=scenarioReasons
    .filter(r=>["BRANCH_EVIDENCE","FIRST_PATH","SECOND_PATH","THIRD_PATH","LINE_ROLE","BRANCH_MASS"].includes(r?.type))
  ...
  const auditBlock=auditReasons.length||extraText
    ?`<details class="scenarioAudit"><summary>詳細な成立根拠を見る</summary><div class="auditMeta">${auditReasons.map(x=>`<p>${esc(x)}</p>`).join("")}${extraText?`<p><b>追加条件内訳</b> ${esc(extraText)}</p>`:""}</div></details>`:"";
  return `<div class="detailBet"><strong>${esc(`${a}-${c}-${d}`)}　${esc(cls)}</strong><div class="purchaseScenario"><p><b>この買い目になるシナリオ</b></p><p>${esc(linkedText)}</p>${integrityOk?`<p class="scenarioLinkOk">予測枝 ${esc(String(integrity.branchId))} → 終端 ${esc(order.join("-"))} に直接紐付け済み</p>`:`<p class="scenarioLinkWarn">この買い目と保存枝の終端が一致しないため、シナリオ説明を断定していません。</p>`}</div><p><b>購入判断：</b>${esc(purchaseSentence)}</p><p class="muted">自然収束度 ${esc(convText)}${Number.isFinite(Number(b?.nodeConditionalProbability))?` / ノード連鎖 ${esc((Number(b.nodeConditionalProbability)*100).toFixed(2))}%`:""}${Number(b?.extraConditionCount)>0?` / 追加条件 ${Number(b.extraConditionCount)}件`:""}${Number.isFinite(Number(b?.extraConditionPenalty))?` / 条件後残存 ${(Number(b.extraConditionPenalty)*100).toFixed(0)}%`:""}${Number(b?.relativeConditionCount)>0?` / 差分条件 ${Number(b.relativeConditionCount)}件・微調整 ${(Math.max(0,1-Number(b.relativeConditionPenalty||1))*100).toFixed(1)}%`:""}${reason?` / ${esc(reason)}`:""}</p>${auditBlock}${Array.isArray(b?.relativeConditionTrace)&&b.relativeConditionTrace.some(x=>Number(x?.count)>0)?`<p class="auditMeta"><b>差分条件内訳</b> ${esc(b.relativeConditionTrace.filter(x=>Number(x?.count)>0).map(x=>`${x.stage}:${x.label||"候補"} 最有力比${(Number(x.ratio||0)*100).toFixed(2)}% / -${(Number(x.penalty||0)*100).toFixed(2)}%`).join(" ｜ "))}</p>`:""}</div>`;
}
...
function renderRatings(panel,snapshot){if(!panel)return;if(!snapshot){panel.classList.add("hidden");panel.innerHTML="";return}const r=ratingOf(snapshot),tone=r.verdictTone||"caution",flags=r.auditFlags||[],idx=Number(r.diagnostics?.evaluationIndex),consistency=r.consistencyAudit||{},adjustments=Array.isArray(consistency.adjustments)?consistency.adjustments:[],checks=Array.isArray(consistency.invariantChecks)?consistency.invariantChecks:[],failed=checks.filter(x=>!x.passed),eligibility=purchaseEligibility(snapshot),quality=r.confidence>=4&&r.concentration>=4?"高":r.confidence<=2||r.concentration<=2?"低":"中",purchase=eligibility.eligible?"購入可":"購入不可",thick=eligibility.eligible&&deriveThickBets(snapshot).length?"厚め候補":eligibility.eligible?"通常":"なし",displayLabel=eligibility.eligible?(quality==="高"?"購入可":"注意"):"見送り";const consistencyHtml=`<div class="evaluationAudit"><strong>評価整合監査: ${esc(consistency.label||"未監査")}</strong>${adjustments.length?`<p>${adjustments.map(esc).join(" / ")}</p>`:""}${failed.length?`<p class="auditWarn">矛盾: ${failed.map(x=>esc(x.label||x.id)).join(" / ")}</p>`:checks.length?'<p>信頼度・集中度・コロがし・最終判定の自動整合チェックを通過。</p>':""}</div>`;panel.className=`card compact ratingPanel rating-${tone}`;panel.innerHTML=`<div class="sectionHead"><div><small>表示用評価・監査中</small><h2>レース評価</h2></div><span class="pill ratingVerdict rating-${tone}">${displayLabel}</span></div><div class="ratingGrid"><div class="ratingItem"><span>予想品質</span><strong>${quality}</strong></div><div class="ratingItem"><span>購入可否</span><strong>${purchase}</strong></div><div class="ratingItem"><span>資金配分</span><strong>${thick}</strong></div><div class="ratingItem"><span>表示判定</span><strong>${displayLabel}</strong></div></div><p class="ratingReason">${esc(r.reason||"")}${Number.isFinite(idx)?` ・ 暫定指数 ${idx.toFixed(1)}`:""}</p>${consistencyHtml}<div class="evaluationAudit"><strong>精度監査: ${esc(r.calibrationLabel||"未校正・検証対象")}</strong>${flags.length?`<p>${esc(flags.join(" / "))}</p>`:'<p>追加フラグなし。ただし的中率・回収率との校正は未実施です。</p>'}</div><p class="muted ratingNote">予想品質・購入可否・資金配分を別軸で表示しています。確率自体は未校正です。</p>`;panel.classList.remove("hidden")}
function noBetReasonText(code){return({NO_TERMINALS:"展開候補を生成できませんでした。",FLAT_DISTRIBUTION_NO_SUPPORTED_CANDIDATE:"確率分布が平坦で、独立した展開根拠を持つ購入候補がありません。",DIFFUSE_NO_NATURAL_BOUNDARY:"上位終端群と残りの間に信頼できる自然境界がなく、予測分布が広いため見送ります。",THIRD_VARIANT_AMBIGUITY:"同じ1-2着内の3着候補を根拠を保ったまま分離できないため、通常購入を見送ります。",BUDGET_BELOW_MINIMUM:"予算が最低購入単位を下回っています。",QUALITY_GATE:"データ品質基準を満たさないため購入を見送ります。",LINE_DATA_UNAVAILABLE:"公式ラインを確認できないため、通常の競輪予想としては購入判定を保留します。"})[code]||"購入価値を確認できないため見送ります。"}
...
function renderSaved(){const all=loadSnapshots(localStorage),localKeys=new Set(all.map(s=>raceKey(s.targetRace)));const remote=state.savedSummaries.filter(s=>!localKeys.has(s.raceKey));const total=all.length+remote.length;$("savedCount").textContent=`${total}件`;const localHtml=all.slice(0,8).map((s,i)=>`<article class="savedItem"><h3>${esc(s.targetRace.venueName)} ${s.targetRace.raceNo}R</h3><p>${formatDate(s.targetRace.date)} / ${formatTime(s.createdAt)} ${s.result?`/ ${resultLabel(s.result.resultStatus)}`:""}</p><button data-saved="${i}">詳細を見る</button></article>`),remoteHtml=remote.slice(0,Math.max(0,8-localHtml.length)).map((s,i)=>`<article class="savedItem"><h3>${esc(s.venue||s.venueName)} ${s.raceNumber||s.raceNo}R</h3><p>${formatDate(String(s.raceKey).slice(0,8))} / ${esc(s.displayStatus||s.lifecycleStatus||"")}</p><button data-sealed-summary="${i}">詳細を見る</button></article>`);$("savedList").innerHTML=total?[...localHtml,...remoteHtml].join(""):'<p class="empty">保存した予想はまだありません。</p>';$("savedList").querySelectorAll("[data-saved]").forEach(b=>b.onclick=()=>{const s=all[Number(b.dataset.saved)];openSavedDetail(s)});$("savedList").querySelectorAll("[data-sealed-summary]").forEach(b=>b.onclick=()=>openSealedSummary(remote[Number(b.dataset.sealedSummary)]));void loadSavedPredictionSummaries()}
async function loadSavedPredictionSummaries(){const date=compact(state.date||localDate());if(state.savedSummaryBusy||state.savedSummaryDate===date)return;state.savedSummaryBusy=true;const started=performance.now();try{const response=await fetch(`/.netlify/functions/keirin-saved-prediction-summary?date=${encodeURIComponent(date)}`,{cache:"no-store",headers:{accept:"application/json"}}),text=await response.text(),networkMs=performance.now()-started,parseStarted=performance.now();let data=null;try{data=JSON.parse(text)}catch{}const jsonParseMs=performance.now()-parseStarted;if(!response.ok||!data?.ok)return;state.savedSummaries=Array.isArray(data.records)?data.records:[];state.savedSummaryDate=date;const renderStarted=performance.now();renderSaved();recordReadMetric({type:"prediction-summary",date,count:state.savedSummaries.length,responseBytes:new TextEncoder().encode(text).byteLength,serverReadMs:data.serverReadMs??null,networkMs:Math.round(networkMs*100)/100,jsonParseMs:Math.round(jsonParseMs*100)/100,renderMs:Math.round((performance.now()-renderStarted)*100)/100,cache:"miss"})}catch{}finally{state.savedSummaryBusy=false}}
function deadlineOf(r){return String(r?.deadline||r?.scheduledStart||"")}
function canonicalReadRaceKey(r){return `${compact(r.date)}-${String(r.venueCode||"").padStart(2,"0")}-${Number(r.raceNo)}`}
function savedLifecycle(r){return(state.collectorStatus?.races||[]).find(row=>row.raceKey===canonicalReadRaceKey(r))||null}
function raceStatus(r){return raceLifecycleView(r,savedLifecycle(r))}
function meetingNextDeadline(m){if(m?.allResultsConfirmed)return"最終R終了 / 本日終了";if(Number.isFinite(Number(m?.finishedRaceCount))&&m?.nextRaceNo)return`${Number(m.finishedRaceCount)}R終了 / 次${Number(m.nextRaceNo)}R`;if(m?.statusLabel)return String(m.statusLabel);return"状態未確認"}
...
function hasUsableGirlsStartPower(a){const e=a?.startPowerEvidence;return Number.isFinite(Number(a?.startPower))&&Boolean(e)&&(!Array.isArray(e.missingInputs)||e.missingInputs.length===0)&&e.profileIdentityPassed!==false}
