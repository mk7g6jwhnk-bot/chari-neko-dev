...
function renderPredictionPerformance(){const root=$("predictionPerformance"),report=state.collectorStatus?.purchasePerformance;if(!root)return;if(!report?.periods){root.innerHTML='<div class="sectionHead"><h2>予想成績</h2><span class="pill">集計中</span></div><p class="muted">正式sealと確定結果が揃うまで「0」ではなく集計中として表示します。</p>';return}const renderPeriod=(label,p)=>{const pct=v=>v===null||v===undefined||!Number.isFinite(Number(v))?"—":`${(Number(v)*100).toFixed(1)}%`,yen=v=>v===null||v===undefined||!Number.isFinite(Number(v))?"—":`${Number(v).toLocaleString("ja-JP")}円`,category=(name,key)=>{const c=p?.categories?.[key];return[`${name}的中率`,pct(c?.hitRate),`${name}回収率`,pct(c?.roi)]};return`<h3>${label}</h3>${metas([["対象",p?.evaluatedRaces??"—"],["購入対象",p?.betRaces??"—"],["的中",p?.hitRaces??"—"],["的中率",pct(p?.hitRate)],["100円平買い投資",yen(p?.investment)],["払戻",yen(p?.return)],["回収率",pct(p?.roi)],category("本線","main").slice(0,2),category("本線","main").slice(2),category("押さえ","cover").slice(0,2),category("押さえ","cover").slice(2),category("厚め","thick").slice(0,2),category("厚め","thick").slice(2)])}`};root.innerHTML=`<div class="sectionHead"><h2>予想成績</h2><span class="pill success">正式sealのみ</span></div>${renderPeriod("今日",report.periods.today)}${renderPeriod("累積",report.periods.cumulative)}`}
...
function renderHomePredictionStatus(){const root=$("homePredictionStatus"),s=state.collectorStatus;if(!root)return;const period=s?.purchasePerformance?.periods?.today,value=n=>n===null||n===undefined?"—":String(n),pct=n=>Number.isFinite(Number(n))?`${(Number(n)*100).toFixed(1)}%`:"—";root.innerHTML=`<div class="sectionHead"><h2>今日の予想状況</h2><span class="pill">100円flat</span></div>${metas([["予想済み",s?.sealedPredictionCount==null?"—":`${s.sealedPredictionCount}R`],["結果待ち",s?.pendingResultCount==null?"—":`${s.pendingResultCount}R`],["的中",period?`${value(period.hitRaces)} / ${value(period.betRaces)}R`:"—"],["100円flat ROI",pct(period?.roi)]])}<p class="muted">実stake成績は保存されたstakeがある場合だけ別集計します。</p>`}
