...
const state={screen:"home",history:[],date:localDate(),meeting:null,meetings:[],meetingTab:"today",race:null,payload:null,snapshot:null,legacySnapshot:null,retry:null,busy:false,sealedLookupBusy:false,sealedLookupRaceKey:null,savedSummaries:[],savedSummaryDate:null,savedSummaryBusy:false,collectorStatus:null,collectorStatusBusy:false,oddsBusyKey:null,bulkBusy:false,bulkDone:0,bulkTotal:0,screeningBusy:false,deepDiveBusy:false,deepDiveCurrentKeys:[]};
...
async function loadCollectorStatus(){if(state.collectorStatusBusy)return;state.collectorStatusBusy=true;let cached=loadCachedCollectorStatus(localStorage);if(cached&&!state.collectorStatus){state.collectorStatus=cached;renderCollectorStatus();renderAutoCollectionSummary();renderPredictionPerformance()}try{const p=await jsonFetch("/.netlify/functions/keirin-collector-status");saveCachedCollectorStatus(localStorage,p);cached=loadCachedCollectorStatus(localStorage);state.collectorStatus=collectorStatusViewModel(p,cached).status||p;renderCollectorStatus();renderAutoCollectionSummary();renderPredictionPerformance()}catch(error){state.collectorStatus=cached?{...cached,statusReadFailed:true,statusReadError:error?.message||String(error)}:{ok:false,statusReadFailed:true,error:error?.message||String(error)};renderCollectorStatus();renderAutoCollectionSummary();renderPredictionPerformance()}finally{state.collectorStatusBusy=false}}
