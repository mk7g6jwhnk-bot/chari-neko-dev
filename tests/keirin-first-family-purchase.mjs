...
assert.equal(by["2-4-1"].purchaseStatus,"購入不採用","終端相対40%の3着違いがv215ボーダーを通過している");
assert.equal(by["2-4-1"].purchaseRejectCode,"PURCHASE_BORDER");
assert.equal(by["2-5-6"].purchaseStatus,"購入不採用","終端相対33%の別2着がv215ボーダーを通過している");
assert.equal(by["2-5-6"].purchaseRejectCode,"PURCHASE_BORDER");
assert.equal(by["2-7-6"].purchaseStatus,"購入不採用","2着の独立支持が弱い終端まで本線に混入している");
assert.equal(by["3-1-5"].purchaseStatus,"購入不採用","主頭比45%未満の別頭が通常押さえに残っている");
assert.equal(by["3-1-5"].purchaseRejectCode,"PURCHASE_BORDER");
...
assert.ok(head2&&head2.main>=1,"本命頭ファミリーの基準終端までボーダーで消えている");
assert.ok(head2.adoptedProbability>0,"本命頭ファミリーの購入確率質量が保存されていない");
assert.ok(head2.adoptedCoverage>.5&&head2.adoptedCoverage<.8,"v215ボーダーよりカバー目標が優先されている");
