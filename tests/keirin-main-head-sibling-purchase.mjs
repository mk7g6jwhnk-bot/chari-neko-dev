...
assert.equal(byOrder.get("1-4-3").betClass,"MAIN","同じ本命展開から自然に収束する本線1頭の2・3着違いが本線に残っていない");
assert.equal(byOrder.get("1-4-3").purchaseStatus,"購入採用");
assert.equal(byOrder.get("1-4-3").adoptionMode,"FIRST_FAMILY_MAIN");
...
assert.notEqual(byOrder.get("2-1-3").adoptionMode,"FIRST_FAMILY_MAIN","別の1着まで本命1着ファミリーとして扱っている");
const diag=purchaseDiagnostics(classified,[],3000);
assert.ok(diag.mainHeadSiblingAudit.candidateCount>=3);
assert.ok(diag.mainHeadSiblingAudit.promotedCount>=1);
