...
assert.equal(byOrder.get("1-4-3").betClass,"COVER","本線1頭の自然な2・3着違いが押さえに昇格していない");
assert.equal(byOrder.get("1-4-3").purchaseStatus,"購入採用");
assert.equal(byOrder.get("1-4-3").adoptionMode,"MAIN_HEAD_SIBLING");
...
assert.notEqual(byOrder.get("2-1-3").adoptionMode,"MAIN_HEAD_SIBLING","別の1着まで本線頭兄弟として扱っている");
