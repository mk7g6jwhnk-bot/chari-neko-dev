...
const storage={data:new Map(),getItem(key){return this.data.get(key)||null},setItem(key,value){this.data.set(key,String(value))}},counts=[];
try{for(const [venueCode,venueName,raceNo] of cases){const response=await handler(new Request(`https://test/.netlify/functions/keirin-predict?${new URLSearchParams({date:"20260808",venueCode,venueName,raceNo:String(raceNo),budget:"3000"})}`)),payload=await response.json();assert.equal(response.status,200);assert.equal(payload.ok,true);assert.ok(payload.prediction.audit.terminalCount>0);assert.equal(payload.prediction.purchasePlan.length,payload.prediction.audit.purchaseCandidateCountAfterCompression);counts.push(payload.prediction.purchasePlan.length);saveSnapshot(storage,createSnapshot(payload));}assert.ok(counts.every(count=>count>0),"終端生成成功レースで買い目0件を禁止");assert.ok(new Set(counts).size>1,"5Rが機械的に同一点数ではない");assert.ok(counts.some(count=>count!==12));assert.equal(loadSnapshots(storage).length,5);}finally{globalThis.fetch=originalFetch}
console.log(`Keirin five-race purchase flow passed: ${counts.join(",")}`);
