...
assert.equal(by["1-4-3"].purchaseStatus,"購入不採用","v215の買い目化ボーダー未達をカバー率だけで復活させている");
assert.equal(by["1-4-3"].purchaseRejectCode,"PURCHASE_BORDER");
assert.ok(by["1-4-3"].purchaseBorderFailures.includes("TERMINAL_RELATIVE"));
assert.equal(by["1-5-3"].purchaseStatus,"購入不採用","買い目化ボーダーよりカバー目標が優先されている");
assert.equal(by["1-5-3"].purchaseRejectCode,"PURCHASE_BORDER");
...
assert.ok(head1.adoptedCoverage<head1.coverageTarget,"ボーダー未達終端までカバー目標のために購入している");
assert.ok(head1.candidateCoverage<head1.coverageTarget,"テスト前提としてボーダー通過候補だけではカバー目標未達であること");
