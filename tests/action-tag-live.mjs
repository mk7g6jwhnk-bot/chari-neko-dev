...
const pending=(await (await fetch(address+'/api/races?reviewer=test-only')).json()).rows; assert.equal(pending.length,0);
const unavailable=(await (await fetch(address+'/api/races?reviewer=test-only&includeUnavailable=1')).json()).rows;assert.equal(unavailable.length,6);assert.ok(unavailable.every(row=>row.availability==='REVIEW_UNAVAILABLE_OLD_VIDEO'));
