...
assert.equal(byOrder["1-2-3"].highPayoutAttributeLabel,"高配当候補");
assert.equal(byOrder["4-5-6"].betClass,"COVER");
assert.equal(byOrder["4-5-6"].highPayoutAttributeLabel,"高配当候補");
assert.equal(byOrder["7-8-9"].betClass,"NONE");
assert.equal(byOrder["7-8-9"].highPayoutAttribute,true);
assert.equal(byOrder["7-8-9"].highPayoutAttributeLabel,"高配当候補");
...
assert.equal(subNoOdds.purchaseRejectCode,"PURCHASE_CUTOFF");
