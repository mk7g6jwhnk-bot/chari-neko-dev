...
import{buildPurchaseEligibility,enforcePostClassificationMainInvariant}from"../keirin/engine/purchase-engine.mjs";
import{allocatePreviewStakes,qualifyThickPredictionBets}from"../public/purchase-funding.mjs";
...
const classified=enforcePostClassificationMainInvariant(classify(terminals,{})).terminals,adopted=classified.filter(x=>x.purchaseStatus==="購入採用");
...
assert.ok(adopted.filter(x=>x.betClass==="COVER").every(x=>x.coverParentOrder&&x.coverParentType&&x.mainDifferenceReason));
assert.equal(adopted.filter(x=>x.betClass==="COVER"&&x.orphanCover===true).length,0,"ORPHAN_COVERは0");
...
assert.equal(allocatePreviewStakes([],3000,"standard"),null,"PURCHASE_BLOCKED時のfundingは0");
