...
async function getJson(base, name, raceKey, fetchImpl = fetch) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const response = await fetchImpl(`${base}/${name}?raceKey=${encodeURIComponent(raceKey)}`, { headers: { accept: 'application/json' }, signal: AbortSignal.timeout(60000) });
      if (!response.ok) throw Error(`HTTP_${response.status}`);
      return await response.json();
    } catch (error) { last = error; if (attempt < 2) await sleep(250 * (attempt + 1)); }
  }
  throw last;
}

export async function fetch50rSource({ cohort, baseUrl = DEFAULT_BASE, fetchImpl = fetch } = {}) {
  if (!cohort || cohort.raceKeys?.length !== 50 || cohort.protectedFinalIncluded !== 0) throw Error('FROZEN_50R_COHORT_REQUIRED');
  const records = [], exclusions = [], hashes = { predictionMismatch: 0, purchaseMismatch: 0, sealedResultMismatch: 0 };
  for (const raceKey of cohort.raceKeys) {
    try {
      ...
      if (prediction.raceKey !== raceKey || result.raceKey !== raceKey) { hashes.predictionMismatch++; exclusions.push({ raceKey, reason: 'RACE_KEY_MISMATCH' }); continue; }
      if (prediction.predictionHash !== result.predictionHash) { hashes.predictionMismatch++; exclusions.push({ raceKey, reason: 'PREDICTION_HASH_MISMATCH' }); continue; }
      if (prediction.predictionHash !== predictionAfter.predictionHash) { hashes.predictionMismatch++; exclusions.push({ raceKey, reason: 'PREDICTION_READBACK_MISMATCH' }); continue; }
      if (hash(result.purchaseEvaluation) !== hash(resultAfter.purchaseEvaluation)) { hashes.purchaseMismatch++; exclusions.push({ raceKey, reason: 'PURCHASE_READBACK_MISMATCH' }); continue; }
      if (result.resultHash !== resultAfter.resultHash) { hashes.sealedResultMismatch++; exclusions.push({ raceKey, reason: 'SEALED_RESULT_READBACK_MISMATCH' }); continue; }
      const sealedAt = Date.parse(prediction.predictionSealedAt), observedAt = Date.parse(result.resultObservedAt);
      records.push({ raceKey, ratingRace: prediction.predictionPayload?.race || {},
    ...
    } catch (error) { exclusions.push({ raceKey, reason: String(error?.message || error) }); }
  }
  const audit = auditThick(records, { sourceTotalV2: cohort.raceKeys.length });
  ...
  return { schemaVersion: 'RECOMMENDATION_THICK_50R_SOURCE_V1', cohort, hashes, exclusions,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const cohort = JSON.parse(await fs.readFile(new URL('./recommendation-thick-50r-cohort.json', import.meta.url)));
  const source = await fetch50rSource({ cohort });
  const target = new URL('./recommendation-thick-50r-source.json', import.meta.url);
