...
export function buildRaceReviewCase({ record, tags = [], coverage = {}, includeOptional = true } = {}) {
  ...
  const girls = participants.length > 0 && participants.every(row => row.raceCategory === "girls" || /^L\d/i.test(String(row.className || "")));
  const questions = RACE_REVIEW_QUESTIONS.filter(question => includeOptional || !question.optional).map(question => ({ ...question, applicability: girls && ["banteResponse","lineState","otherLineSurvival"].includes(question.id) ? "NOT_APPLICABLE" : "APPLICABLE", suggestedValue: suggestion(question.id, automatic), targetRiderId: target(question.id, initiative, bante)?.riderId || null, coverageGap: coverageGap(question.id, coverage) }));
  return Object.freeze({ version: RACE_REVIEW_VERSION, reviewMode: girls ? "GIRLS" : "STANDARD", caseId: id("CASE", record.raceKey), raceKey: record.raceKey, createdAt: record.reviewQueuedAt || new Date().toISOString(), lineup: lineSummary(participants), initiativeCandidate: initiative, banteCandidate: girls ? null : bante, automaticCandidates: automatic.map(compactTag), preRace: Object.freeze({ predictionSealedAt: record.predictionSealedAt || null, venue: record.venueName || record.venue || null, raceNo: record.raceNo || null }), evidence: evidenceRows(record, automatic), questions, decisionCount: questions.length, productionUi: false });
}
...
export function submitRaceReview(reviewCase, answers = {}, options = {}) {
  ...
  for (const question of reviewCase.questions) {
    if (question.applicability === "NOT_APPLICABLE") continue;
...
function normalizeParticipants(record) { const rows = record.participants || record.sealed?.participants || [], lines = record.lines || record.sealed?.lines || []; const lineMap = new Map(lines.map(row => [Number(row.number), row])), sizes = new Map(); for (const row of lines) sizes.set(String(row.lineId), (sizes.get(String(row.lineId)) || 0) + 1); return rows.map((row, index) => { const line = lineMap.get(Number(row.number)) || row, lineId = line.lineId == null ? null : String(line.lineId), linePosition = Number(line.position ?? line.linePosition ?? row.linePosition) || null, lineSize = Number(row.lineSize) || sizes.get(lineId) || null; return { riderId: String(row.registration || row.riderId || row.id || row.number), number: Number(row.number ?? index + 1), name: row.name || null, className: row.className || row.class || null, raceCategory: String(row.raceCategory || '').toLowerCase(), lineId, linePosition, lineSize, role: row.role || (linePosition === 1 ? "LINE_HEAD" : linePosition === 2 ? "BANTE" : null), conditionalCells: [linePosition === 1 ? "LINE_LEADER" : linePosition === 2 ? "BANTE" : null, lineSize === 2 ? "TWO_RIDER_LINE" : lineSize >= 3 ? "THREE_PLUS_RIDER_LINE" : null].filter(Boolean) }; }); }
