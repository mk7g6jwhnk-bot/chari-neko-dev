...
export function observeActionTags(record, { now = () => new Date().toISOString() } = {}) {
  ...
  const attackValue = ({ ESCAPE:"ESCAPE_SUCCESS", MAKURI:"MAKURI_SUCCESS", DIFFERENCE:"DIFFERENCE_SUCCESS", MARK:"MARK_FINISH" })[method];
  if (winner && attackValue) tags.push(direct(common, winner, "ATTACK_OUTCOME", attackValue, observationTime, official, method));
...
function normalizeMethod(value) { const text = String(value || "").trim().toLowerCase(); if (/^捲り?$|^まくり$|^makuri$/.test(text)) return "MAKURI"; if (/^逃げ?$|^nige$|^escape$/.test(text)) return "ESCAPE"; if (/^差し?$|^sashi$|^difference$/.test(text)) return "DIFFERENCE"; if (/^マーク$|^mark$/.test(text)) return "MARK"; return null; }
