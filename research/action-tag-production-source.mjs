...
export class ProductionActionTagSource {
  constructor({ store, baseUrl = DEFAULT_BASE, fetchImpl = fetch, maxPerRun = 5, timeoutMs = 30000, maxHeapBytes = 192 * 1024 * 1024 } = {}) {
    Object.assign(this, { store, baseUrl: baseUrl.replace(/\/$/, ''), fetchImpl, maxPerRun, timeoutMs, maxHeapBytes });
    this.metrics = { mode:'READ_ONLY_PRODUCTION_SOURCE_LOCAL_RESEARCH_SINK', attempted:0, accepted:0, duplicates:0, excluded:0, failures:0, http502:0, http503:0, timeouts:0, predictionHashMismatch:0, purchaseHashMismatch:0, peakHeapBytes:0, lastError:null, startedAt:null, completedAt:null };
  }
  async run() {
    this.metrics.startedAt = new Date().toISOString();
    const status = await this.get('keirin-collector-status');
    const candidates = (status.races || []).filter(row => row.resultLifecycleState === 'RESULT_CONFIRMED' && Date.parse(row.resultObservedAt) >= Date.parse(this.store.enrollment.startedAt)).sort((a,b) => Date.parse(a.resultObservedAt)-Date.parse(b.resultObservedAt));
    for (const row of candidates) {
      ...
      if (await this.store.getRace(row.raceKey)) { this.metrics.duplicates++; continue; }
      ...
      try {
        ...
        if (await this.store.ingest(meta, record)) this.metrics.accepted++; else this.metrics.duplicates++;
      ...
      } catch (error) { this.metrics.failures++; this.metrics.lastError=String(error?.message || error); }
      await yieldTurn();
    }
  ...
  async get(path) {
    let response;
    try { response = await this.fetchImpl(`${this.baseUrl}/${path}`, { headers:{accept:'application/json'}, signal:AbortSignal.timeout(this.timeoutMs) }); }
    catch (error) { if (/timeout|abort/i.test(String(error))) this.metrics.timeouts++; throw error; }
