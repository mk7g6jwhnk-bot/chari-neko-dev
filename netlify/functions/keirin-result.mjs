...
export default async function handler(req){
  ...
  // 旧Railway版互換。必ず resultOnly=1 を付け、重い通常予想経路を使わない。
  try{
    const fallbackQ=new URLSearchParams(q);
    fallbackQ.set("resultOnly","1");
    const response=await fetch(`${base}/keirin/race?${fallbackQ}`,{headers:{accept:"application/json"},signal:AbortSignal.timeout(22000)});
    let data=null;try{data=await response.json()}catch{}
    attempts.push({path:"/keirin/race?resultOnly=1",status:response.status,error:data?.error||null});
    const result=normalizeResult(data?.result||data?.officialData?.result||data?.officialResult);
    if(response.ok&&result)return jsonResponse(200,{ok:true,race:{date,venueCode,venueName,raceNo},result,checkedAt:data?.checkedAt||new Date().toISOString()});
    if(response.status===404||response.status===405)return jsonResponse(409,{ok:false,error:"公式結果取得サービスに結果専用経路がありません。Railway側の結果取得機能を更新してください",attempts});
    return jsonResponse(409,{ok:false,error:data?.error||"公式結果は未確定、または結果取得サービスが一時的に利用できません",attempts});
  }catch(error){
    attempts.push({path:"/keirin/race?resultOnly=1",error:error instanceof Error?error.message:String(error)});
...
export function normalizeResult(value){
  ...
  const combination=finishOrder.join("-"),trifecta=Array.isArray(value.payouts?.trifecta)?value.payouts.trifecta.find(x=>String(x.combination||"").replace(/\D/g,"")===combination.replace(/\D/g,""))||value.payouts.trifecta[0]:null;
  return{status,finishOrder,payout:Number(value.payout||value.trifectaPayout||trifecta?.payout||0)||null,source:value.source||value.sourceType||"official"};
}
