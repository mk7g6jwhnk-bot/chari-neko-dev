...
export default async req => {
  ...
  try {
    ...
    for (const m of schedule.meetings.slice(0, 20)) {
      try {
        const r = await fw(m.discoveredUrl, jar, scheduleUrl);
        ...
        meetings.push({ ...m, verifiedMeeting: true, discoveryError: e.message, discovery: { ok: false, links: { raceCards: [], odds: [], results: [], other: [] }, diagnostics: { fallback: true } } });
      }
    }
    ...
    return jsonResponse(200, {
      ...
      date,
      meetings,
      diagnostics: { ...schedule.diagnostics, cookieNames: jar.names(), note: "対象日セル＋KEIRIN.JP内部リンクのみ採用" },
...
async function fw(url, jar, referer = null) {
  const h = { "user-agent": "Mozilla/5.0 (compatible; ChariNekoDev/0.5; personal-use)", "accept-language": "ja" };
