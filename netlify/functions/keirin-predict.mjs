...
function buildDisplayRatingInputs(payload={}){
  ...
  return{version:"DISPLAY-RATING-INPUTS-1.0",terminalProbabilitySum:finiteRatingValue(prediction.audit?.terminalProbabilitySum),top3Mass:finiteRatingValue(prediction.audit?.top3Mass),top5Mass:finiteRatingValue(prediction.audit?.top5Mass),branchConcentrationRaw:finiteRatingValue(diagnostics.branchConcentrationRaw),terminalConcentrationRaw:finiteRatingValue(diagnostics.terminalConcentrationRaw),oddsEvaluation:{adoptedCandidateCount:adopted?.length??null,evaluatedCount,totalCandidateCount:standard.length,allOddsEvaluated:Boolean(diagnostics.allOddsEvaluated),maxExpectedValueIndex:finiteRatingValue(diagnostics.maxExpectedValue)},massStatus:diagnostics.massStatus||null,availability:{branchConcentrationRaw:Number.isFinite(Number(diagnostics.branchConcentrationRaw)),terminalConcentrationRaw:Number.isFinite(Number(diagnostics.terminalConcentrationRaw)),oddsEvaluation:Boolean(adopted),massStatus:Boolean(diagnostics.massStatus)}};
}
